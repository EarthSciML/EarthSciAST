...
*Transitivity (normative occurrence sites).* A mount-edge rename rewrites the declaration key AND every reference to the old name inside the mounted document. The list is §9.7.7's — `index_sets` registry key; `of` parent lists of ragged/derived definitions; `{"from": <name>}` references; the axis-naming scalar fields `wrt`, `dim` and `integral`'s `var`, plus an `integral` `lower`/`upper` bound that is a **bare string naming a renamed index set**; and `where` `shape` constraint entries (§9.6.1) — plus the three sites that exist only because a mount carries a whole component rather than a set of declarations: **`ModelVariable` / `Parameter` `shape` entries**, **`Assertion.coords` KEYS** (§6.6.5), and **`DataSourceSelectAxis.gated_by`** (§8.9.2). Never rewritten: `from_faq` and expression-node `id` (node identities, not axis names); a ragged set's `offsets` / `values` / `member_factor` (keyed-factor variable names — §9.7.7's `rebind` domain); `aggregate` `output_idx` entries and `ranges` KEYS (bound index symbols, never index-set names); `coordinates.<k>.source` (a data-array name); and the §9.7.7 protected structural scalars.
...
*Where it applies (binding status).* This section's mount edge is the §4.7 `subsystems.<k> = {ref}` edge, which every binding resolves at the mount and merges here. The top-level `models.<k>` / `reaction_systems.<k>` `{ref}` mount is the same schema shape (`SubsystemRef`) and takes the same field with the same meaning, but that form's index-set merge is not yet uniform — **Python** merges the leaf's `index_sets`, **Julia** and **Rust** splice the leaf and defer its `expression_template_imports` to the root pass instead, and **TypeScript** and **Go** do not inline a top-level `{ref}` stub at all. `index_set_rename` at a top-level model-ref mount therefore follows that merge wherever it lands; it is normative for the subsystem edge in every binding today.
