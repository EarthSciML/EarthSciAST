...
5. **Option A expands CALL SITES. It does NOT delete DECLARATIONS.** A top-level `expression_templates` registry and a top-level `metaparameters` block (§9.7.1) are *declarations* — peers of `index_sets` — not `apply_expression_template` call sites. They **survive `parse → emit` verbatim**, and a **template-library file MUST round-trip to itself**. This is pinned because every binding got it backwards: they expanded the call sites *and* dropped the registry, so a pure library file emitted as `{esm, metadata, index_sets}` — a document carrying **none** of the five top-level payload keys, which the schema's top-level `anyOf` rejects. Combined with rule 4 (schema validation runs post-expansion) that made a conforming library file **unrepresentable**: legal on disk, illegal the moment it was loaded and re-emitted. A document kind that cannot round-trip to itself is not a document kind. `tests/valid/template_import_lib.esm` and `tests/valid/template_import_rename_lib.esm` pin the shape; both are schema-VALID as authored, and their emitted form MUST be too.
