...
|---|---|---|
| `D` | `"wrt": "t"` or a spatial axis | Derivative ∂/∂(`wrt`) of `args[0]`. `wrt:"t"` (or an absent `wrt`) is the **structural** time derivative and is **strictly unary**: on an equation LHS it is consumed by system assembly; in a right-hand side it **names the tendency** of the unknown in `args[0]` and resolves to that unknown's own `D(x)/dt` right-hand side (see "A right-hand-side structural `D`" below). A **spatial** `wrt` is a **rewrite-target** (§9.6.8): lowered to a stencil by a discretization rule, never evaluated directly; it MAY carry **trailing auxiliary operands** after `args[0]` (see below). A right-hand-side structural `D` that resolves to no tendency is a rewrite-target too. |
...
**A right-hand-side structural `D` is the named unknown's tendency (normative).** A `D` with
`wrt:"t"` (or no `wrt`) whose `args[0]` is a **bare reference to an unknown that carries a
differential equation** denotes that unknown's tendency: the right-hand side of its own
`D(x)/dt ~ f`. It is **resolved by substitution during flattening** (§4.7.5) — the tendency
replaces the `D` node in the canonical flattened form — so no evaluator ever meets one, and
every consumer of the flattened system answers alike without knowing the rule. Substitution
**recurses** (a tendency may itself name another state's derivative, `D(lai)/dt ~ sla ·
D(biomass)/dt`); a self- or mutually-referential chain terminates with the node left as
authored, which the gate below then catches.
...
about reactions. The value is not a runtime's to invent: the unknown already has a defining
equation in the same system, so this is an ordinary AST substitution, not differentiation.

Scope is exactly that. The format defines **no symbolic differentiation**, so a structural `D`
of an **observed**, of a **parameter**, or of a **compound expression** resolves to nothing and
remains a rewrite-target — an implementation MUST NOT invent a value for it (in particular
**not `0`**, which is a wrong answer rather than a missing one), and the `unlowered_operator`
gate (§9.6.3 constraint 6) rejects it before evaluation exactly as it rejects an undiscretized
spatial `D`. Nothing here changes the LHS: a `D` on an equation's left-hand side is what makes
that equation differential and is never rewritten.
