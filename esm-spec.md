...
*When the check is evaluated (normative).* On the **AUTHORED** document, before any §4.7 mount edge is resolved. This is not an implementation convenience; both halves of the accepted set are only visible there. A §4.7 mount CONSUMES the leaf's `metaparameters` at its edge (§9.7.6 site 3), so once refs are resolved the names a conforming `extent` legitimately reaches are gone and every such document would be refused. Resolving refs ahead of the root's own §9.7 machinery is a legitimate thing for a binding to do — a rewrite target inside a mounted component only lowers if the content is spliced in first — so a binding MUST evaluate this check before that point rather than wherever its root close happens to sit. Nor is "after the close" an alternative: the close folds this document's own `index_sets` sizes to integers, which satisfies the resolved-shape exemption below for any document that mounts nothing, and the purest typo of all — a single file whose `extent` misspells a metaparameter the file itself declares — would stop being caught.
