...
**Two mount forms, one mechanism.** A `ref` object is admissible at two attachment points: a `subsystems.<k>` entry (above), and a **top-level `models.<k>` / `reaction_systems.<k>` entry** — the schema gives each of those blocks `oneOf [<component>, SubsystemRef]`. The second form mounts the referenced component as a *top-level system* of the assembling document, under the map key, which is what lets a coupled document name its components by file and have its `coupling` endpoints (`<key>.<var>`) resolve with no rewriting. **The two forms differ only in where the mounted component lands.** Everything else this section states applies to both identically: the reference formats and `${VAR}` expansion, the single-component requirement and the `model` / `reaction_system` selector, `bindings` (§9.7.6 site 3), the §9.7.10 edge injection, the resolution-timing rule — **and the index-set merge below**. Two mount forms that merged different things would make an assembly's meaning depend on which attachment point the author happened to pick; a binding MUST NOT make them differ. Current bindings: all five mount a top-level `models.<k>` ref, and all five run the edge pipeline below at both forms, closing and folding the leaf at the mount. One gap remains, at the top-level `reaction_systems.<k>` form, which only **Julia** mounts at all: that inliner is still a raw pre-pass that defers all of §9.7 to the root document's resolution, so a mounted reaction system's own `metaparameters` are neither closed by the edge's `bindings` nor folded at the mount. The other four leave such an entry unresolved. A `size` naming something the leaf does not declare behaves one of two ways, and the split is a binding-observable consequence of §9.7.6 site 3 rather than a rule stated here: a leaf carrying NO §9.7 machinery has no close to run, so the `size` merges symbolically and is closed by the mounting document's own §9.7.6 close, which is what lets an assembler-scoped axis keep working; a leaf carrying ANY machinery closes strictly at the edge, where such a name is `metaparameter_unbound` and never reaches the mounting document. Every binding agrees on the strict half. The symbolic half they do NOT: **Python**, **Rust** and **TypeScript** carry such a `size` through the merge as written, while **Julia** at a `subsystems.<k>` mount and **Go** at either mount form cannot represent it — their index-set declaration types hold an integer `size` — and abort the mount with a raw language-level error rather than a diagnostic. Among the three that do merge it, WHERE the merge sits relative to the mounting document's own §9.7.6 close is a second, verdict-bearing disagreement: Rust and Julia merge before that close, so an assembly that also declares the metaparameter may restate the leaf's symbolic axis verbatim and still be deep-equal, while Python and TypeScript merge after it and see the restatement already folded to a number, which is `subsystem_index_set_conflict`. Deleting the restatement loads everywhere the symbolic merge is representable at all. Whether the edge close SHOULD be strict about a name only the assembler can bind, and which side of that merge order is right, are both open — see `docs/content/rfcs/mount-edge-index-set-renaming.md` §6.
