...
*Which environment a contribution folds against.* The environment is the one belonging to **whatever registry the contribution lands in**, not the one belonging to the document that started the load. A contribution merging into a MOUNTED leaf's own registry — a nested mount's axes joining the leaf that mounts them — folds against that LEAF's closed environment; a contribution merging into the root's registry folds against the root's. The two coincide only at depth zero, and getting it wrong is silent: a nested axis sized by a name only the leaf declares stays symbolic if it is folded against the root's environment instead, and then either travels up unfolded or is closed at the wrong value by a same-named metaparameter the assembler happens to declare.
