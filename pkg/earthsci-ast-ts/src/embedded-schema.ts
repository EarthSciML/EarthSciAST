  ...
  "$defs": {
    ...
    "ReactionSystem": {
      ...
      "properties": {
        ...
        "tolerance": {
          "$ref": "#/$defs/Tolerance",
          "description": "System-level default numerical tolerance for tests. Resolution is PER FIELD (esm-spec 6.6.4): abs and rel each take their value from the innermost of assertion / test / system that declares that field, so a test or assertion that declares only one bound does not mask the other from here. An explicit 0 is a declaration ('no bound of this kind'), not an absence, and stops the fallthrough."
        },
        "tests": {
          "type": "array",
    ...
    "Assertion": {
      ...
      "properties": {
        ...
        "tolerance": {
          "$ref": "#/$defs/Tolerance",
          "description": "Per-assertion tolerance override, merged PER FIELD over the test-level and model-level defaults (esm-spec 6.6.4): each of abs and rel that this object declares wins, and each it omits falls through independently. Declaring only abs here therefore keeps an outer rel rather than dropping it; write rel: 0 to mean 'no relative bound'."
        },
        "coords": {
          "type": "object",
    ...
    "Test": {
      ...
      "properties": {
        ...
        "tolerance": {
          "$ref": "#/$defs/Tolerance",
          "description": "Test-level default tolerance, merged PER FIELD (esm-spec 6.6.4): it supplies each of abs and rel that an assertion in this test does not declare, and falls through to the model-level default for each field it does not declare itself."
        },
        "expression_template_imports": {
          "type": "array",
