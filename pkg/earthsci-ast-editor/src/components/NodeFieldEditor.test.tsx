...
import type { ExpressionNode as ExprNode } from '@earthsciml/ast'
import {
  NodeFieldEditor,
  opHasFieldEditor,
  buildUpdatedNode,
...
/**
 * Build an ExprNode fixture from a loose op literal, centralising the
 * `as unknown as ExprNode` cast the ExprNode union would otherwise force at
 * every call site.
 */
function makeNode(node: { op: string; args?: unknown[]; [key: string]: unknown }): ExprNode {
...
describe('buildUpdatedNode round-trips', () => {
  it('edits a const value (number)', () => {
    const node = makeNode({ op: 'const', args: [], value: 0 })
  ...
  it('edits D wrt while preserving args', () => {
    const node = makeNode({ op: 'D', args: ['u'], wrt: 't' })
  ...
  it('edits enum type and member into args[0] / args[1]', () => {
    const node = makeNode({ op: 'enum', args: ['Type', 'member'] })
  ...
  it('edits table_lookup axes (JSON map) and keeps args empty', () => {
    const node = makeNode({ op: 'table_lookup', args: [], table: 't', axes: {} })
  ...
  it('parses a comma-separated shape for reshape', () => {
    const node = makeNode({ op: 'reshape', args: ['a'], shape: [1] })
  ...
  it('edits the manifold of intersect_polygon', () => {
    const node = makeNode({ op: 'intersect_polygon', args: ['a', 'b'], manifold: 'spherical' })
  ...
  it('preserves an unrelated field such as id', () => {
    const node = makeNode({ op: 'D', args: ['u'], wrt: 't', id: 'node-1' })
    const result = buildUpdatedNode(node, { wrt: 'x' })
...
describe('serializeField', () => {
  ...
  it('reads enum args into synthetic fields', () => {
    const node = makeNode({ op: 'enum', args: ['Season', 'summer'] })
...
describe('NodeFieldEditor component', () => {
  ...
  it('keeps the form open and shows an error on invalid JSON', () => {
    ...
    render(() => <NodeFieldEditor node={node} path={[]} onReplace={onReplace} onClose={onClose} />)
