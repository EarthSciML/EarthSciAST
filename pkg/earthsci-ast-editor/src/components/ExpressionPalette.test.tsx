...
describe('ExpressionPalette', () => {
  ...
  describe('Search functionality', () => {
    it('filters templates based on search query', () => {
      const onInsert = vi.fn()
      render(() => <ExpressionPalette searchQuery="exp" onInsertExpression={onInsert} />)
    ...
    it('shows no results message when search has no matches', () => {
      const onInsert = vi.fn()
      render(() => <ExpressionPalette searchQuery="nonexistent" onInsertExpression={onInsert} />)

      expect(screen.getByText('No expressions found for "nonexistent"')).toBeInTheDocument()
      expect(
        screen.getByText('Try searching for operators, functions, or keywords'),
      ).toBeInTheDocument()
    })
