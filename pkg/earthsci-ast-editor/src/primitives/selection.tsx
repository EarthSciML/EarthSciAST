/**
 * Selection and inline editing primitives for the earthsci-ast-editor
 *
...
export function createSelectionContext(
  ...
  const selectedNodeDetails = createMemo((): NodeDetails | null => {
    ...
    const value =
      typeof expression === 'object' && 'op' in expression
...
export function SelectionProvider(props: SelectionProviderProps) {
  ...
  return (
    <SelectionContext.Provider value={contextValue}>{props.children}</SelectionContext.Provider>
  )
}
 ...
 */
export function getVariableSuggestions(esmFile: EsmFile | null, searchTerm: string = ''): string[] {
  const allVars = extractVariableNames(esmFile)

  if (!searchTerm) return allVars

  const lowerTerm = searchTerm.toLowerCase()
  return allVars.filter((variable) => variable.toLowerCase().includes(lowerTerm))
}
