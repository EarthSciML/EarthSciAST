...
// TestAggregateValidFixtures asserts every tests/valid/faq/*.esm fixture
// parses and schema-validates cleanly through the Go loader. These exercise the
// additive aggregate/semiring schema deltas (op:"faq", the closed
...
func TestAggregateValidFixtures(t *testing.T) {
	...
	pattern := filepath.Join(repoRoot, "tests", "valid", "faq", "*.esm")
...
// TestAggregateInvalidFixtures asserts every tests/invalid/faq/*.esm
...
func TestAggregateInvalidFixtures(t *testing.T) {
	...
	pattern := filepath.Join(repoRoot, "tests", "invalid", "faq", "*.esm")
	files, err := filepath.Glob(pattern)
	if err != nil {
		t.Fatalf("glob %s: %v", pattern, err)
	}
...
// into this component's scope — so a `faq` range naming a set the
...
func TestAggregateUndeclaredRangeStillRejected(t *testing.T) {
	...
	path := filepath.Join(repoRoot, "tests", "invalid", "faq", "undeclared_from_name.esm")
...
func TestIndexSetsDocumentScopeRoundTrip(t *testing.T) {
	...
	cases := []struct {
		...
		{
			rel:      filepath.Join("tests", "valid", "faq", "faq_semiring_indexset.esm"),
	...
	for _, tc := range cases {
		t.Run(filepath.Base(tc.rel), func(t *testing.T) {
			...
			// carry `faq` / `intersect_polygon` op nodes whose required
