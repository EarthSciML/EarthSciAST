...
binding (``pkg/EarthSciAST.jl/test/relational_test.jl``) and the
...
def test_adapter_matches_golden_manifest():
    ...
    from earthsci_ast.cli.determinism_adapter import _compute_fixture
...
def test_determinism_runner_accepts_python_adapter(tmp_path):
    ...
    env["EARTHSCI_DETERMINISM_ADAPTER_PYTHON"] = (
        f"{sys.executable} -m earthsci_ast.cli.determinism_adapter"
    )
