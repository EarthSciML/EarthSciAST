"""
Tests that the new STAC-like DataSource schema can express every data loader
implemented in EarthSciData.jl.

Each fixture under tests/fixtures/data_sources/*.esm hand-constructs an
...
2. It round-trips through parse -> serialize without loss of DataSource fields.
...
def test_earthscidata_fixture_roundtrips(fixture_name):
    """Each fixture's DataSource block must survive parse -> serialize unchanged."""
    fixture_path = FIXTURES_DIR / fixture_name
    ...
    out_loaders = out_data.get("data_sources", {})
