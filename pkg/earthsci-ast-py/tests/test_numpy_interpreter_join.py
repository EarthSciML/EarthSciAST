...
def test_join_resolved_through_simulate_pipeline() -> None:
    ...
    res_join = solve(
        esm_problem(load_document(_join_count_model(with_join=True)), (0.0, 1.0), u0={"u": 0.0})
    )
    res_full = solve(
        esm_problem(load_document(_join_count_model(with_join=False)), (0.0, 1.0), u0={"u": 0.0})
    )
