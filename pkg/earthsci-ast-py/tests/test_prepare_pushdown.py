...
def test_prepare_pushdown_record_gate_end_to_end(oracle):
    ...
    # The fixture froze the SAME LCC projection parameters the oracle recomputes.
    # These cannot be compared bit-exactly: the fixture is generated by
    # scripts/generate-pushdown-goldens.jl, and Julia's and CPython's libm
    # disagree by a few ULP on this log/tan/cos chain (n by 3 ULP, rho0 by 5).
    # A tight relative tolerance still catches any real edit to LAT1/LAT2/LAT0/
    # LON0, which moves these constants by many orders of magnitude.
    _vars = doc["models"]["ISRM"]["variables"]
    for _key, _want in zip(("lcc_n", "lcc_rf", "lcc_rho0", "lcc_lam0"), oracle["C"]):
        assert _vars[_key]["default"] == pytest.approx(_want, rel=1e-12, abs=0), _key
...
def test_observed_field_reports_hoist_skip_root_cause(oracle):
    ...
    try:
        prep = esm_problem(
            doc, (0.0, 1.0), providers=providers, const_arrays=ca, pushdown_rewrite=True
        )
