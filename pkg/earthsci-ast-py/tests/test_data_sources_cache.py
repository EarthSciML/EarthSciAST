...
* **Unit** — the content-addressing, EARTHSCIDATADIR resolution, offline
  ...
  :mod:`earthsci_ast.data_sources.cache`. These use fake openers/fetchers
...
from earthsci_ast.data_sources import cache as cache_mod
...
def test_existing_default_opener_path_unchanged(tmp_path):
    ...
    dl = DataSource(
        ...
        source=DataSourceLocation(url_template="mem://{date:%Y%m%d}.nc"),
        temporal=DataSourceTemporal(file_period="P1D"),
    )
    # A source declares no variables from 1.0.0: the binding belongs to the
    # consuming parameter (which also owns the units), and the reader is handed
    # it by the caller that knows the consumers.
    bindings = {"u": DataSourceBinding(file_variable="U")}
    ...
    res = GridLoader(dl, bindings).load(time=dt.datetime(2018, 11, 8), opener=lambda _u: _DS())
