...
def resolve_template_machinery(
    ...
    # binding site 1) — the imported subtree is instantiated with those values and
    # the name is consumed there. It does NOT enter the importing document's own
    # scope: site 2 re-exports only the names an edge leaves OPEN. So a bare name
    # in the consumer's own expressions that merely coincides with an edge binding
    # is a free variable reference and MUST stay symbolic (Julia, the reference
    # binding, plus Go and TypeScript all leave it symbolic). Folding it here would
    # also bypass the §9.7.6 `metaparameter_name_conflict` no-shadowing check —
    # a leaked binding could silently replace a *declared* variable of the same
    # name with an integer literal. A consumer that genuinely wants to share a size
    # with the library declares its own `metaparameters` entry and passes it down
