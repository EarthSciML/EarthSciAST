...
from ..esm_types import DataSource
...
def resolve_source_urls(
    ...
    *,
    ...
    """Expand ``data_source``'s source URL (plus mirrors) for ``time``.
    ...
    if time is not None and data_source.temporal and data_source.temporal.file_period:
        anchor = file_anchor_for_time(
            time,
            file_period=data_source.temporal.file_period,
            start=data_source.temporal.start,
        )
