...
:class:`LoadDataProvider` — backed by :func:`data_sources.runtime.load_data` —
...
:class:`~earthsci_ast.data_sources.grid.GridLoadResult`; for a real
...
class LoadDataProvider:
    ...
    def _resolve_io(self):
        ...
        if self._use_cache:
            from ..esm_types import DataSourceKind
            from .cache import cached_fetcher, cached_opener

            if self.field.data_source.kind == DataSourceKind.POINTS:
    ...
    def _temporal(self) -> Any:
        return self.field.data_source.temporal
    ...
    def _bindings(self):
        """The ONE binding this field reads, keyed by the file variable's own
        name so the loader returns it under the key
        :func:`simulation._provider_array` looks for. From esm 1.0.0 the binding
        lives on the consuming parameter (esm-spec §8.5), and a provider serves
        exactly one parameter, so there is exactly one."""
        from ..esm_types import DataSourceBinding

        return {
            self.field.var: DataSourceBinding(
                file_variable=self.field.var,
                unit_conversion=getattr(self.field, "unit_conversion", None),
            )
        }
...
def build_default_provider(
    ...
    url_substitutions = _static_url_substitutions(field.data_source, target)
