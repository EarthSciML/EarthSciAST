...
/// ranges resolve cleanly through *this* bead's resolver — `op:"faq"`,
...
fn shared_valid_aggregate_fixture_parses_and_resolves() {
    let file = common::load_repo_fixture("valid/faq/faq_semiring_indexset.esm");
...
/// invalid fixture under tests/invalid/faq/ is SCHEMA-VALID (so `load`
...
fn shared_invalid_undeclared_from_fixture_is_rejected() {
    ...
    let file = common::load_repo_fixture("invalid/faq/undeclared_from_name.esm");
...
/// `{from}` to a `RaggedDyn`, `extract_derivative_faq` builds a ragged
