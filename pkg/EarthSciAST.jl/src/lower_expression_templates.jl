...
`expression_from_json` applies at the AST-literal boundary (`parse.jl`), so the two
...
→ canonical sorted write) never runs `expression_from_json`, so without this pass a
...
no-op-equivalent: `coerce_esm_file` reads operands via `expression_from_json`, which
...
function lower_expression_templates(raw_data)
    ...
    # integer, so the AST-golden pathway (which never calls `expression_from_json`)
...
function _expand_expr_refs(e::OpExpr, reg, sites::_TemplateSites,
    ...
    if e.op == APPLY_EXPRESSION_TEMPLATE_OP
        ...
        ex = _expand_expr_refs(expression_from_json(substituted), reg, sites, memo)
