...
@testset "EarthSciAST.Expression Operations" begin
    ...
    @testset "substitute function" begin
        ...
        num = NumExpr(3.14)
        bindings = Dict{String,EarthSciAST.Expr}("x" => NumExpr(2.0))
        ...
        diff_expr = OpExpr("D", EarthSciAST.Expr[var_x], wrt="t", dim="time")
            ...
            @testset "$filename" begin
                ...
                for (i, case) in enumerate(cases)
                    ...
                    @testset "$label" begin
                        ...
                        bindings = Dict{String,EarthSciAST.Expr}(
    ...
    @testset "free_variables function" begin
        ...
        repeated = OpExpr("+", EarthSciAST.Expr[VarExpr("x"), VarExpr("x"), VarExpr("y")])
    ...
    @testset "simplify function" begin
        ...
        @test simplify(OpExpr("+", EarthSciAST.Expr[NumExpr(2.0), NumExpr(3.0)])) == NumExpr(5.0)
        @test simplify(OpExpr("*", EarthSciAST.Expr[NumExpr(2.0), NumExpr(3.0)])) == NumExpr(6.0)

        # Test additive identity: x + 0 = x
        var_x = VarExpr("x")
        @test simplify(OpExpr("+", EarthSciAST.Expr[var_x, NumExpr(0.0)])) === var_x
        @test simplify(OpExpr("+", EarthSciAST.Expr[NumExpr(0.0), var_x])) === var_x

        # Test additive identity with all zeros
        @test simplify(OpExpr("+", EarthSciAST.Expr[NumExpr(0.0), NumExpr(0.0)])) == NumExpr(0.0)

        # Test multiplicative identity: x * 1 = x
        @test simplify(OpExpr("*", EarthSciAST.Expr[var_x, NumExpr(1.0)])) === var_x
        @test simplify(OpExpr("*", EarthSciAST.Expr[NumExpr(1.0), var_x])) === var_x

        # Test multiplicative zero: x * 0 = 0
        @test simplify(OpExpr("*", EarthSciAST.Expr[var_x, NumExpr(0.0)])) == NumExpr(0.0)
        @test simplify(OpExpr("*", EarthSciAST.Expr[NumExpr(0.0), var_x])) == NumExpr(0.0)

        # Test multiplicative identity with all ones
        @test simplify(OpExpr("*", EarthSciAST.Expr[NumExpr(1.0), NumExpr(1.0)])) == NumExpr(1.0)

        # Test exponentiation rules
        @test simplify(OpExpr("^", EarthSciAST.Expr[var_x, NumExpr(0.0)])) == NumExpr(1.0)
        @test simplify(OpExpr("^", EarthSciAST.Expr[var_x, NumExpr(1.0)])) === var_x
        @test simplify(OpExpr("^", EarthSciAST.Expr[NumExpr(0.0), NumExpr(2.0)])) == NumExpr(0.0)
        @test simplify(OpExpr("^", EarthSciAST.Expr[NumExpr(1.0), var_x])) == NumExpr(1.0)
        ...
        nested = OpExpr("*", EarthSciAST.Expr[OpExpr("+", EarthSciAST.Expr[NumExpr(1.0), NumExpr(2.0)]), var_x])
        ...
        n_ary_add = OpExpr("+", EarthSciAST.Expr[var_x, NumExpr(0.0), VarExpr("y"), NumExpr(0.0)])
    ...
    @testset "substitute edge cases" begin
        ...
        var_x = VarExpr("x")
        var_y = VarExpr("y")
        circular_bindings = Dict{String,EarthSciAST.Expr}(
            "x" => var_y,
            "y" => var_x,
        )
        ...
        inner_x_plus_one = OpExpr("+", EarthSciAST.Expr[var_x, NumExpr(1.0)])
        nested_self = Dict{String,EarthSciAST.Expr}("x" => inner_x_plus_one)
        ...
        deep_expr = var_x
        for i in 0:(depth - 1)
            deep_expr = OpExpr("+", EarthSciAST.Expr[deep_expr, VarExpr("v$i")])
        end
        deep_bindings = Dict{String,EarthSciAST.Expr}("x" => NumExpr(1.0))
        ...
        empty_op = OpExpr("+", EarthSciAST.Expr[])
        any_bindings = Dict{String,EarthSciAST.Expr}("x" => NumExpr(42.0))
        ...
        compound = OpExpr(
            ...
            EarthSciAST.Expr[
                var_x,
                OpExpr("+", EarthSciAST.Expr[var_y, NumExpr(1.0)]),
            ];
            wrt="t",
            dim="time",
        )
        empty_bindings = Dict{String,EarthSciAST.Expr}()
        ...
        d_expr = OpExpr("D", EarthSciAST.Expr[var_x]; wrt="t", dim="time")
        num_bindings = Dict{String,EarthSciAST.Expr}("x" => NumExpr(3.14))
    ...
    @testset "Integration tests" begin
        ...
        expr = OpExpr("*", EarthSciAST.Expr[OpExpr("+", EarthSciAST.Expr[VarExpr("x"), NumExpr(0.0)]), VarExpr("y")])
        bindings = Dict{String,EarthSciAST.Expr}("y" => NumExpr(1.0))
