...
@testset "coords sampling — nearest index, exact ties round DOWN" begin
    ...
    for r in results
        @test r.passed
        @test r.status == EarthSciAST.PASS   # shared AssertionStatus enum
...
@testset "PdeAssertionResult status enum + JUnit emission" begin
    ...
    results = _pit_run(file)
    @test length(results) == 3
    @test results[1].status == EarthSciAST.PASS
    @test results[1].passed
    @test results[2].status == EarthSciAST.FAIL
    @test !results[2].passed
    @test results[2].actual !== nothing
    @test results[3].status == EarthSciAST.ERROR
...
@testset "§6.6.5 reference binds the field's dimension names" begin
    ...
    for reg in (Dict{String,Any}("lev" => [1.0]), Dict{String,Any}("M.lev" => [1.0]))
        ...
        @test_throws EarthSciAST.PdeTestError EarthSciAST.bind_dimension_names(
