...
WHY THESE FIVE AND NOTHING ELSE. Everything in between — `_scalar_op`'s broadcast ladder,
...
import EarthSciAST: _oop_read_state, _oop_gather, _oop_du_zeros, _oop_store,
    ...
    _knot_count, _knot_pair, _knot_pair2, _bilinear_corners,
    _scan_lanes_oop, _ScanFold, _oop_new_memo, _oop_intern_tally!,
    _scalar_op, _oop_const, _oop_powlit, _oop_gvn_tally!,
    _read_param_data
...
# `_scalar_op`/`_oop_const` seams in tree_walk/scalar_ops.jl and oop.jl. What lives here is the part
...
#
# THE FOLD ORDER IS THE LADDER'S. `_scalar_op(:+)` folds LEFT — `((c1+c2)+c3)…` —
...
function _scalar_op(op::Symbol, c::AbstractVector, ::Type{T}, memo::_RxGatherMemo) where {T}
