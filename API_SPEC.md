...
- **Julia's build/inspection seam** — `BuildInspection`, `evaluate_expr`,
  ...
  `remake_parameters`. (`build_evaluator` left it — §8 item 23.)
...
value invention → gated fetch → compile). `build_evaluator` has **left the
surface** (§8 item 23): it is private behind `esm_problem`, with a deprecated
alias for one minor. What used to come off it comes off the Problem —
...
**This alias is not exported**, which is the one way it departs from the rule
above, and the departure is deliberate rather than an oversight. The rule's
purpose is that a downstream keeps compiling while it migrates, and the sole
downstream (ReSEACT, plus `EarthSciASTDiff`) reaches the name through a
qualified `EarthSciAST.build_evaluator` / `EA.build_evaluator`, which a
non-exported binding still answers. Keeping it in the export list would keep it
in the manifest and in §6's count — that is, still *on the surface* — which is
the thing item 23 retires. A caller who did rely on the bare name adds the
qualification, or, better, moves to `esm_problem`.
