...
- **Required targets need no new syntax:** a coupling target declared without a `default` makes an
  ...
  `coupling_import`'s expanded edges, so a mis-bind is reported by `validate` on the source document
  — naming the import, the role, the bound component and the missing variable — as well as at
  flatten.
