...
- **Future work, not yet in any binding's array evaluator.** Running discrete events and solving implicit equations on the array path is unimplemented in Julia (tree-walk), Python (NumPy interpreter) and Rust alike; each refuses both with `unsupported_construct` instead (conformance category `tests/conformance/unsupported_construct/`). Julia's ModelingToolkit export is the one runner that executes them today.
