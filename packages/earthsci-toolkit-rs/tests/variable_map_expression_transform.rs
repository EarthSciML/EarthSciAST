...
use earthsci_toolkit::{FlattenError, flatten, validate};
use serde_json::json;
...
fn factor_with_expression_transform_rejected_by_load() {
    ...
    let err = earthsci_toolkit::load(&doc.to_string())
...
fn lower_templates_expands_coupling_transform_against_receiving_component() {
    ...
    earthsci_toolkit::lower_expression_templates::lower_expression_templates(&mut doc)
...
fn simulate_variable_map_expression_transform_end_to_end() {
    use earthsci_toolkit::{SimulateOptions, simulate};
