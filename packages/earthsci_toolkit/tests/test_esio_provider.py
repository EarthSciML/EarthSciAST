...
def test_geotiff_loader_resolves_bbox_and_builds_provider():
    ...
    field = _field(
        _LANDFIRE_URL,
        fmt_meta=_LANDFIRE_META,
        variables=("fuel_model",),
        file_vars={"fuel_model": "Band1"},
        name="LANDFIRE.fuel",
    )
...
def test_cds_loader_window_trims_to_calendar_month_and_days():
    ...
    field = _field(
        "https://data.earthsci.dev/era5/x.nc", temporal=t, fmt_meta=meta, variables=("u", "t")
    )
    target = _Target([-121.6, -121.4], [39.7, 39.9])
...
@pytest.mark.skipif(
    not (pathlib.Path.home() / ".cdsapirc").exists(), reason="no ~/.cdsapirc credentials present"
)
