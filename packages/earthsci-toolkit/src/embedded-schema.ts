  ...
  "$defs": {
    ...
    "ExpressionTemplate": {
      ...
      "properties": {
        "params": {
          "type": "array",
          "description": "Ordered list of parameter (metavariable) names. MUST be unique within this template. Each name occurs zero or more times inside `body` and (when present) `match`; for an explicitly-invoked template every name MUST also appear as a key in every `apply_expression_template.bindings` referencing it.",
          "items": {
            "type": "string",
            "minLength": 1
          },
        ...
        "match": {
          "$ref": "#/$defs/Expression",
          "description": "Optional pattern Expression that turns this template into an auto-applied rewrite rule. Parameter names appearing as bare strings in `match` are wildcards bound by structural matching. Absent ⇒ the template is applied only via explicit `apply_expression_template` invocation."
        },
        "priority": {
          "type": "integer",
          "default": 0,
