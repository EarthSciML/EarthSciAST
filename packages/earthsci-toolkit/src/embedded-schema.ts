...
export const schema: AnySchemaObject = {
  ...
  "anyOf": [
    ...
    },
    {
      ...
      "properties": {
        "kind": {
          "const": "grid_discretization_descriptor"
        }
      }
    }
  ],
  "properties": {
    ...
    "kind": {
      "type": "string",
      "description": "Optional document-kind discriminator. When set to 'grid_discretization_descriptor', the document is a GridDiscretization Descriptor (GDD) file (esm-spec.md §4.7.1). GDD files carry grid geometry and discretization rules together and are referenced from test and example grid_refs fields. Absent for standard ESM component documents."
    },
    "metadata": {
      "$ref": "#/$defs/Metadata"
    },
  ...
  "$defs": {
    ...
    "Equation": {
      ...
      "properties": {
        ...
        "region": {
          "$ref": "#/$defs/RuleRegion",
          "description": "Optional spatial scope. When present, marks this equation as scoped to a region (e.g. a boundary or initialization point); the discretization engine dispatches it to region-specific rules rather than the interior rule pass."
        }
      }
    },
    "AffectEquation": {
    ...
    "Assertion": {
      ...
      "properties": {
        ...
        "expected_order": {
          "type": "number",
          "description": "Required when `reduce` is `convergence_order`. The expected numerical convergence rate (e.g., 2.0 for second-order methods). The assertion passes when the measured rate p_obs >= expected_order * (1 - tolerance.rel)."
        }
      },
      "allOf": [
        {
        ...
        "grid_refs": {
          ...
          "items": {
            ...
            "properties": {
              "ref": {
                "type": "string",
                "description": "URL or path to a GridDiscretization Descriptor (.gdd.json) file."
              }
            }
          },
          "minItems": 1
    ...
    "Example": {
      ...
      "properties": {
        ...
        "initial_state": {
          ...
          "description": "Initial conditions for state variables. Reuses the top-level InitialConditions $def (constant / per_variable / from_file / expression). For PDE components the `expression` variant carries per-variable closed-form initial fields (see esm-spec.md §11.4)."
        },
        "parameters": {
          "type": "object",
    ...
    "BoundaryCondition": {
      ...
      "properties": {
        ...
        "flux_match": {
          "type": "boolean",
          "description": "For kind='interface': if true, enforce normal-flux continuity (∂u/∂n = ∂v/∂n) in addition to value continuity at the interface. Default false.",
          "default": false
        },
        "face_coords": {
          "type": "array",
    ...
    "GridMetricGenerator": {
      ...
      "properties": {
        ...
        "name": {
          "type": "string",
          "description": "For kind='builtin': canonical name from the closed set defined in §6.4 (currently empty). Unknown names MUST be rejected with E_UNKNOWN_BUILTIN."
        }
      },
      "allOf": [
        {
    ...
    "GridCRS": {
      ...
      "properties": {
        "projection": {
          ...
          "description": "Projection family. `longlat` is the geographic identity case (no projection); the others are projected CRSs whose `parameters` drive the downstream reprojection rule."
        },
        "datum": {
          "type": "string",
          "enum": [
          ...
          "description": "Geodetic datum. `sphere` is a spherical Earth of radius `R` (required when `datum=sphere`); `WGS84` is the standard ellipsoid."
        },
        "R": {
          "type": "number",
          "exclusiveMinimum": 0,
          "description": "Sphere radius in metres, used when `datum=sphere` (e.g. WRF 6370000, NEI2016 6370997)."
        },
        "parameters": {
          "type": "object",
          "description": "Projection parameters consumed verbatim by the downstream reprojection rule for this `projection` (e.g. Lambert Conformal `{lat_1, lat_2, lat_0, lon_0}`). Absent/empty for `longlat`.",
          "additionalProperties": {
            "type": "number"
          }
        }
      ...
      "oneOf": [
        ...
        },
        {
          ...
          "properties": {
            ...
            "side": {
              "type": "string",
              "description": "For kind='boundary': the boundary side (e.g. 'xmin', 'xmax', 'west', 'east', 'south', 'north', 'top', 'bottom'). Site-specific sides are permitted when declared by the grid.",
              "enum": [
                "xmin",
                "xmax",
                "ymin",
        ...
        "stencil_gen": {
          ...
          "properties": {
            "method": {
              "type": "string",
              "const": "fornberg",
              "description": "Weight-generation algorithm. Only \"fornberg\" is supported (Fornberg 1988 recurrence, exact rational arithmetic)."
            },
            "deriv_order": {
              "type": "integer",
              "minimum": 1,
              "description": "Derivative order (1 = first derivative / gradient). Only 1 is currently supported; higher orders land in follow-on bead ess-5b."
            },
            "accuracy_order": {
              "type": "integer",
              "minimum": 2,
              "multipleOf": 2,
              "description": "Truncation error order for centered differences (must be a positive even integer: 2, 4, 6, 8, …). Determines stencil width: accuracy_order/2 points on each side of the evaluation point."
            },
            "stagger": {
              "type": "string",
              "enum": [
              ...
              "description": "Stencil symmetry. Only \"centered\" is currently supported (symmetric about the evaluation point, zero-weight center). One-sided / upwind stagger lands in follow-on bead ess-5b."
            },
            "axis": {
              "type": "string",
              "description": "Cartesian axis along which the stencil is applied. May be a literal dimension name or a pattern variable (e.g. \"$x\") resolved against the triggering rule's bindings at expansion time."
            },
            "spacing": {
              "type": "string",
              "description": "Spacing mode. For uniform grids: a symbol name (e.g. \"dx\") used as the denominator variable in the rational coefficient AST — coeff = numerator / (denominator * spacing). For non-uniform grids: the literal string \"from_grid\", which causes the loader to generate symbolic references to per-location float weight arrays (__stgfw_{axis}_{accuracy_order}_{offset}) that are computed from actual grid node coordinates at ODE-build time."
            }
          }
        },
        "axes": {
    ...
    "DiscretizationVariant": {
      ...
      "properties": {
        ...
        "axes": {
          ...
          "description": "Variant: dimensional_split axes."
        },
        "inner_rule": {
          "type": "string",
    ...
    "MultiOutputStencilRule": {
      ...
      "properties": {
        "kind": {
          ...
          "description": "Discriminator for bindings that need to distinguish this rule type from a standard Discretization. Optional (presence of `outputs` plus an object-valued `stencil` is sufficient), but recommended for statically-typed bindings."
        },
        "applies_to": {
          "description": "Shallow (depth-1) AST pattern identifying the operator this scheme discretizes (e.g. reconstruct($q, dim=$x)). Guard only — bindings flow from the triggering rule by name (RFC §7.2.1). Same shape as Discretization.applies_to.",
          "$ref": "#/$defs/PatternNode"
        },
        "grid_family": {
          "type": "string",
          ...
          "description": "Grid family this scheme targets. When present, the stencil selectors must be compatible with this family."
        },
        "outputs": {
          "type": "array",
          "minItems": 1,
          ...
          "description": "Ordered list of named outputs emitted by this scheme. MUST equal the union of keys in `stencil` and keys in `derived` (loader-enforced: E_OUTPUTS_STENCIL_MISMATCH on violation). Names must be unique within the array. Consumers reference these names in their `requires` map as '<this-scheme>#<output>'."
        },
        "stencil": {
          "type": "object",
          "minProperties": 1,
          "additionalProperties": {
            ...
            "description": "§7.1 stencil-entry list for this output. Each entry is a {selector, coeff} pair following the same rules as Discretization.stencil entries."
          },
          "description": "Object keyed by output name (vs §7.1's flat array). Each value is a list of §7.1 stencil entries ({selector, coeff}) that expand at $target to produce the named output. Key set MUST be a subset of the `outputs` array; the remaining outputs must appear in `derived` (loader-enforced: E_OUTPUTS_STENCIL_MISMATCH)."
        },
        "derived": {
          "type": "object",
          "additionalProperties": {
            "$ref": "#/$defs/ExpressionNode",
            "description": "An ExpressionNode over other stencil output names and the operand. Each bare output-name reference is replaced at expansion time by an indexed reference to the mangled emitted variable."
          },
          "description": "RFC §7.9 OQ3 — derived (non-stencil) outputs. Object keyed by output name; each value is an ExpressionNode whose free variables are names declared in `stencil` (the stencil outputs). Derived outputs are emitted as pointwise arrayop equations where the RHS references the stencil outputs by their mangled names. No name may appear in both `stencil` and `derived` (loader-enforced: E_DERIVED_STENCIL_OVERLAP). The union of `stencil` keys and `derived` keys MUST equal the `outputs` array (loader-enforced: E_OUTPUTS_STENCIL_MISMATCH)."
        },
        "emits_location": {
          "type": "string",
          "description": "Default staggered-grid location for all emitted output variables (e.g. 'face', 'cell_center', 'edge_normal'). Individual outputs may override this via a per-output location declaration (v2 extension, deferred). When absent, location defaults to the triggering operand's location."
        },
        "primary": {
          "oneOf": [
            {
          ...
          "description": "Name of the output to substitute at the matched expression site in the consumed-directly trigger path. MUST be null or absent for provider-only schemes (those with no `applies_to` match in any model equation — they fire only via consumer `requires`). When non-null, MUST name a declared output (loader-enforced: E_PRIMARY_NOT_AN_OUTPUT)."
        },
        "accuracy": {
          "type": "string",
          "description": "Informational: truncation order (e.g. 'O(dx^4)')."
        },
        "order": {
          "type": "integer",
          "minimum": 1,
          "description": "Optional scalar order selector (mirrors Discretization.order)."
        },
        "requires_locations": {
          "type": "array",
          "items": {
        ...
        "free_variables": {
          ...
          "description": "Optional list of free pattern-variable names (e.g. ['$q', '$x']) that the scheme expects the triggering rule to bind; informational / for validator use."
        },
        "description": {
          "type": "string"
        },
    ...
    "GridDiscretizationDescriptor": {
      "type": "object",
      "description": "A GridDiscretization Descriptor (GDD) document (esm-spec.md §4.7.1). GDD files are identified by kind='grid_discretization_descriptor' and carry a grids block (spatial grid geometry per domain) plus an optional discretizations block (operator rules). They are referenced exclusively from test and example grid_refs fields to run a PDE component on a specific grid configuration.",
      "required": [
        "esm",
        "kind"
      ],
      ...
      "properties": {
        ...
        "kind": {
          "type": "string",
          "const": "grid_discretization_descriptor",
          "description": "Document-kind discriminator. Must be 'grid_discretization_descriptor'."
        },
        "metadata": {
          "$ref": "#/$defs/Metadata"
        },
        "grids": {
          "type": "object",
          "description": "Dictionary mapping grid/domain names to grid specifications. Keys must match the grid or domain name used by the referencing component. Each value overrides the component's declared grid for that name when the test or example runs.",
          "additionalProperties": true
        },
        "discretizations": {
          "type": "object",
          "description": "Dictionary mapping operator names to discretization rules or {ref} pointers to ESD rule files (§4.7.1). Overrides the component's declared discretizations for this run.",
          "additionalProperties": {
            "oneOf": [
              {
                "$ref": "#/$defs/Discretization"
              ...
              {
                "type": "object",
                "description": "Reference to an external ESD rule file.",
                "required": [
                  "ref"
                ],
                "additionalProperties": false,
                "properties": {
                  "ref": {
                    "type": "string",
                    "description": "URL or path to an ESD rule file whose discretizations block is merged into the component's discretizations."
                  }
                }
              }
            ]
    ...
    "DiscretizationRef": {
      ...
      "properties": {
        "ref": {
          "type": "string",
          "description": "Local file path or URL pointing to an ESD rule file (an ESM file whose discretizations block contains exactly one scheme definition). The scheme is loaded and used as the inline definition for this discretizations entry."
        }
      }
    }
  }
