...
const _BI_ESS = EarthSciSerialization
...
@testset "BuildInspection — per-pair regrid setup arrays (exact rationals)" begin
    ...
    @test insp.observed_exprs isa Dict{String,EarthSciSerialization.Expr}
...
@testset "ragged CSR contraction — expression position + observed assertion" begin
    ...
    file = EarthSciSerialization.load(IOBuffer(JSON3.write(doc)))
