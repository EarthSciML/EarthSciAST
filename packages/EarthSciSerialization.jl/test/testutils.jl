# Shared test prelude for the EarthSciSerialization.jl suite.
#
...
# (`julia --project -e 'using EarthSciSerialization, Test; include("test/<file>")'`)
...
# ---------------------------------------------------------------------------
_n(x) = EarthSciSerialization.NumExpr(Float64(x))
_i(x) = EarthSciSerialization.IntExpr(Int64(x))
_v(n) = EarthSciSerialization.VarExpr(String(n))
_op(op, args...; kw...) =
    EarthSciSerialization.OpExpr(String(op), EarthSciSerialization.Expr[args...]; kw...)
...
_ao1(body, idx, lo, hi) = EarthSciSerialization.OpExpr("arrayop",
...
_const(val) = EarthSciSerialization.OpExpr("const",
    EarthSciSerialization.Expr[]; value=val)
...
function _stencil_model(N)
    vars = Dict("u" => EarthSciSerialization.ModelVariable(
        EarthSciSerialization.StateVariable))
    body = _op("+",
        _idx("u", _op("-", _v("i"), _i(1))),
        _op("*", _n(-2.0), _idx("u", _v("i"))),
        _idx("u", _op("+", _v("i"), _i(1))))
    EarthSciSerialization.Model(vars, [EarthSciSerialization.Equation(
