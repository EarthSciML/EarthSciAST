...
#### 5.22.2 ⛔ The pin is the resolved path, not a successful read
...
### 5.38 Compiled Right-Hand-Side Conformance (normative)

§5.9 governs the **interpreters'** agreement on a discretized RHS and a
trajectory. This section governs the **compiled backends**: the two emitters —
Julia's direct StableHLO emission through the Reactant extension, and Rust's
XlaBuilder emission through the `xla` crate — must reproduce the **reference
interpreter's** right-hand side `f(u, p, t)` at fixed probe states, within a
written per-class tolerance. It is simultaneously the cross-binding gate for the
three interpreters themselves (Julia reference, Rust, Python), so it runs before
either compiled lane exists and grows into the compiled gate as they land.

Shared fixtures, probes, tolerance classes and exclusions live in
`tests/conformance/compiled_rhs/manifest.json`, driven by
`scripts/run-compiled-rhs-conformance.py`. The manifest is generated by
`scripts/gen_compiled_rhs_manifest.py`; the tier's contract (manifest schema,
adapter CLI, output JSON) is `tests/conformance/compiled_rhs/README.md`.

Go and TypeScript are **out of scope** — they implement only the rewrite half
(no `faq`/`makearray` evaluator, so no RHS hook to probe). Python has **no
compiled backend** in this plan; it participates with its interpreter only.

#### 5.38.1 What is compared

One quantity, and nothing else: the **right-hand side** `f(u, p, t)` at each
declared probe, keyed by bare column-major element name (`u[1]`, `u[i,j]`, a
scalar `s`; the model namespace stripped). Each probe fixes a complete flat state
vector — every element of the fixture's `state_order`, written **literally** in
the manifest — and a time `t`, so every binding and every engine evaluates the
identical input.

Each engine's output is gated against **two** references:

1. the committed **Julia-interpreter golden** (`golden/<id>.json`), which lies
   entirely outside every compiled path, and
2. the probe's **`analytic_rhs`** anchor, computed from the fixture's
   mathematics **without any binding** — an operator matrix `L u + b` for the
   pre-discretized fixtures, a closed form for the rest.

> ⛔ **Agreement is NUMERICAL, never structural.** Nothing in this tier inspects
> an emitted program. An emitted StableHLO module and an emitted XLA computation
> have no reason to match each other or to match anything, and a byte comparison
> on either MUST NOT be asserted. What is asserted is that the numbers land
> inside the band.

#### 5.38.2 The four tolerance classes

Every fixture declares exactly one `tolerance_class`. A value passes when

```
|got − want| ≤ atol + rtol · |want|
```

| Class | rtol | atol | Applies to |
|---|---|---|---|
| `algebraic` | 1e-13 | 1e-300 | `+ − × ÷`, stencils, gathers — arithmetic whose reassociation is the only freedom |
| `transcendental` | 1e-12 | 1e-300 | `cos`, `sin`, `exp`, `log`, `sqrt`, `atan2`, `^` — a decade of slack for libm differences between a host libm and an XLA lowering |
| `reduction` | 1e-11 | `atol_scaled · maxᵢ|wantᵢ|` | sums and other folds, whose error grows with the summation order the emitter chooses |
| `float32` | 1e-5 | 1e-30 | reserved for precision-changing fixtures; unused while §5.38.4 excludes them |

The `reduction` class's absolute floor is **scaled**, not fixed: `atol =
atol_scaled · maxᵢ|wantᵢ|` over the **whole probe vector**, with the scale taken
from the reference so it is the same number for every binding. A reduction
legitimately produces exact zeros beside large entries (a stencil interior that
cancels, a filtered `faq` row that sums to nothing), and a purely relative bound
on an exact zero is an impossible bound. `atol_scaled = 1e-14` on a probe whose
largest entry is 1e3 is an absolute floor of 1e-11.

A fixture MAY carry a tighter or looser bound than its class only with a written
reason in its manifest entry.

#### 5.38.3 A refusal is a hard error, and a NAMED exclusion

A compiled engine that cannot lower a model completely **MUST refuse it** —
`{"status": "refused", "rule": …, "reason": …}` for that fixture — and MUST NOT
fall back to an interpreter, a partial lowering, or a host callback. A silent
fallback would make this tier report the interpreter agreeing with itself, which
is precisely the reading the outcome exists to prevent.

The runner then splits on the fixture's `compiled_required` list:

* a refusal from a binding the fixture **lists** → the gate **FAILS**;
* a refusal from a binding it does **not** list → reported as a **named
  exclusion**, with the binding, the fixture, the rule and the reason, in the
  report and on the console. It is never a pass and never a silent skip.
...
`Float64`, is **excluded by name** in the manifest's `excluded` array with the
reason *"precision-changing; excluded until both compiled emitters lower it
(ruling 2026-09-13)"*. Neither emitter lowers a precision change today, and an
emitter that silently widened one to `Float64` would produce a **better** answer
than the reference — the hardest kind of wrong to notice, since it looks right
(the same failure §5.19.3a pins for the recurrence fold). The exclusion is a
ledger, not a licence: it shrinks as the emitters gain coverage, and an excluded
fixture never reports as passing.

Any other fixture the tier cannot carry is listed in the same array with its own
reason and what would have to change to include it.

#### 5.38.5 Shape

| Comparison | Shape (`tests/conformance/README.md`) |
|---|---|
| compiled engine vs golden | **reference-comparing** — the golden is the Julia *interpreter*, wholly outside every compiled path |
| any engine vs `analytic_rhs` | **reference-comparing** — an anchor computed independently of every binding |
| Rust / Python interpreter vs golden | **cross-binding-agreeing** |
| Julia interpreter vs golden | a regression check against its own committed output — which is why every fixture that *can* carry an `analytic_rhs` anchor MUST, and why `--self-test` gates the golden against those anchors on every run |

#### 5.38.6 Gate

`scripts/run-compiled-rhs-conformance.py --self-test` is the always-on regression
guard, and needs no live binding: it asserts the committed golden reproduces
every `analytic_rhs` anchor, and that the harness **rejects** a value moved 10x
its tolerance budget, a missing element, a missing probe, and a refusal from a
`compiled_required` binding — while **reporting** a refusal from a binding that
is not required as a named exclusion.

The producer modes run from `test-conformance.sh`: `--engine interpreter` for
Julia, Rust and Python (all three `bindings_required`), and `--engine compiled`
for Julia and Rust (both `bindings_optional` until phase 2, so an `unavailable`
...
**Status.** Julia, Rust and Python each answer for `interpreter` and `native`
on all six fixtures with no refusals, and all three are `bindings_required` for
each; Python is `bindings_required` for `sympy`, which refuses the four array
fixtures by name. **Rust is `bindings_required` for `xla` as of 2026-09-22**:
`esm_problem(…, compiler = Xla)` emits the model's tape as an XLA computation
and solves on the compiled right-hand side, and all six fixtures agree with the
golden with no refusals. That entry carries a BUILD condition — `xla` needs the
crate's non-default `xla` feature and a separately fetched `xla_extension` — so
"required" means a build that has both must answer, and a stage that runs this
compiler owes them. Julia's `xla`, and `mtk`, remain `bindings_optional`.
...
| The registry holds the **US customary family — `ft`, `mi`, `lb`, `hp`, `gal`, `short_ton`, `tonne`, `inHg`** — and deliberately **no `ton`, no `t`, no `in`, no `yd`, no `mph`** | §4.8.1 | The FF10 point-source format stores stack geometry in feet and the annual total in short tons; MOVES stores link length in miles, link speed in mi/h, engine ratings in horsepower, its emission rates in g/(hp*h), fuel consumption in lb/(hp*h), fuel density in g/gal and barometric pressure in inHg. Without those entries a document must either lie (`kg/yr` on a short-ton column: dimensionally perfect, wrong by 907.18474, invisible) or be rejected. `ton` is excluded because it is three different masses; `t` because a one-letter mass symbol reads as tera-; `in`/`yd` because no corpus column uses them; `mph` because it is a fused spelling of a compound the grammar already builds. Scales are pinned EXACTLY, and the SCALE is the contract wherever two conventions share a dimension: `ft` = 0.3048 m, `mi` = 1609.344 m (= 5280 ft), `lb` = 0.45359237 kg, `short_ton` = 907.18474 kg (= 2000 lb), `hp` = 745.6998715822702 W (MECHANICAL, 550 ft*lbf/s — the metric PS is 735.49875 W, 1.4% away), `gal` = 0.003785411784 m³ (US LIQUID — the imperial gallon is 4.54609 L, 20% away), `inHg` = 3386.388640341 Pa (= 25.4 mmHg). `short_ton` and `tonne`, `hp` and PS, `gal` and the imperial gallon each share a dimension, so only the scale distinguishes them. Fixtures: `tests/valid/units_inventory_registry.esm`, `tests/valid/units_moves_registry.esm`, `tests/conformance/unit_registry`. |
