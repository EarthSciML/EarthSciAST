...
│   ├── fixtures/<id>.esm           #   the three documents authored here (four more are REFERENCED)
│   └── golden/<id>.json            #   the Julia-INTERPRETER actual for every assertion
├── scalar_operator_semantics/      # what each scalar core operator COMPUTES (CONFORMANCE_SPEC §5.45.4)
│   ├── README.md                   #   the contract, and the three named exclusions it found
...
| `compiled-RHS compiled producer (julia/rust)` — **where it is required** | The two stages above are the SAME stages in two workflows, gated differently, and the difference is deliberate. In `.github/workflows/conformance-testing.yml` neither compiled engine is provisioned (no `xla` cargo feature, no `XLA_EXTENSION_DIR`, no `ESM_TEST_REACTANT`), so each **skips visibly** — legal because `manifest.json` leaves `engines.compiled.bindings_required` empty and lists julia and rust as `bindings_optional`. Rust skips through the adapter, which answers `unavailable` when it was built without the feature; julia skips because `scripts/test-conformance.sh` gates its stage on `ESM_TEST_REACTANT` — without that gate the adapter's self-bootstrapping environment would *install* Reactant and run the stage for real, since the adapter reserves `unavailable` for Reactant failing to load. In `.github/workflows/xla-backends.yml` both engines ARE provisioned and the skip is a **failure**: that workflow runs `scripts/assert-compiled-rhs-available.py` over the report and fails unless the binding's status is literally `ok`, because the runner's exit code cannot distinguish "passed" from "was never asked". A **refusal** (a model the emitter cannot lower) fails in BOTH workflows wherever the fixture names the binding in its `compiled_required`; optionality is about availability only |
| `compiler-agreement self-test` | reference-comparing (the committed goldens must reproduce every anchor each fixture carries from the tier it came from; the negative controls assert the harness rejects a value outside its band, a missing element, a missing save time, and a refusal from a `required` binding) |
