...
| FORMAT-08-A-011 | The discovered count binds at §9.7.6 site 4, and that site reaches any document in the assembly that declares the name — this document OR one it MOUNTS at either §4.7 mount form, transitively. A mounting document MUST NOT have to restate a metaparameter its leaf already declares | esm-spec.md §8.9.4, §9.7.6 | Yes | validation |
| FORMAT-08-A-012 | An `extent.metaparameter` declared by neither this document nor any document it mounts MUST be rejected at LOAD with `template_import_unknown_name`, not deferred to the sample at build time | esm-spec.md §8.9.4, §9.6.6 | Yes | validation |
| FORMAT-08-A-013 | A leaf mounted at a `subsystems.<k>` edge and the same leaf mounted at a top-level `models.<k>` edge MUST be sized from the discovered extent IDENTICALLY (§4.7 "Two mount forms, one mechanism"); a binding that backfills the loader-API bindings at one attachment point and not the other sizes one from the data and the other from the leaf's placeholder default, silently | esm-spec.md §4.7, §8.9.4 | Yes | behavioral |
