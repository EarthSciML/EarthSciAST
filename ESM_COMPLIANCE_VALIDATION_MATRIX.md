...
| BEHAV-06-B-018 | An inline-test assertion's `variable` MAY be a scoped reference (schema `Assertion.variable`; issue #263), resolved by the rule the asserting component's own equations use: a dotted name whose HEAD is a subsystem key of the asserting component is relative to it (`Leaf.u` in `Host` with a `Leaf` subsystem is `Host.Leaf.u`), and any other dotted name is document-absolute (§4.6). Every lookup the assertion makes — the trajectory row, an array state's cells, an observed's field, the declared shape a `coords` map is checked against — MUST be made against the component that OWNS the name. Before the fix a `coords` / `reduce` assertion on a mounted leaf's field errored with `variable 'Leaf.key' is not declared in model 'Host'` in both mount forms, and a pointwise one on a name two components answer to read whichever row sorted first. Julia (`inline_tests.jl::_resolve_asserted_name`), Python (`inline_tests.py::_resolve_asserted_name`) and Rust (`inline_tests.rs::resolve_asserted_name`): FIXED. Rust also chose its simulation backend (`file_has_array_ops`, `file_has_spatial_model`, `has_differential_equations`) from top-level models only, so a document whose only derivative or array lives in a `subsystems` mount failed to build; those now ask the question of the whole mount tree (`simulate_array::model_tree_any`). TypeScript and Go have no inline-test runner and do not resolve `Assertion.variable` at validation, so there is nothing to diverge. Gate: `tests/conformance/scoped_assertion_variable/` — shared fixtures driven per binding (`scoped_assertion_variable.rs`, `test_scoped_assertion_variable.py`, `scoped_assertion_variable_test.jl`) over a top-level mount, a `subsystems` mount, and a document where both readings of `Leaf` exist and the subsystem must win | esm-spec.md §6.6.3, §4.6 | Yes | simulation |
