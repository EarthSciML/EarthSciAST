...
| BEHAV-13-003 | The resolved value MUST carry through ARITHMETIC and `join.on` key comparison as an ordinary number. The shared fixture `tests/valid/enums_zero_and_negative.esm` pins `Braking + 10*Idling + Unassociated = 0 + 10 − 1 = 9`, which a binding that clamped or dropped a sign cannot produce | esm-spec.md §9.3; CONFORMANCE_SPEC.md §5.26.3 | Yes | behavioral |
...
| BEHAV-13-005 | Values MUST remain unique within one enum, and `0` is a value like any other — two symbols MAY NOT both map to `0`. Not expressible in JSON Schema, so it lives in each loader | esm-spec.md §9.3; CONFORMANCE_SPEC.md §5.26.4 | Yes | validation |
