...
>
> **BEHAV-04-D-003/004 — mount-edge `index_set_rename` (2026-09-06, issue #198 item 4; RFC `docs/content/rfcs/mount-edge-index-set-renaming.md`)**: implemented in **all five** at the §4.7 subsystem edge — Julia `template_imports.jl::apply_mount_index_set_rename` (from `resolve.jl::_lower_and_coerce`, with the map carried on `SubsystemRef.index_set_rename`), Python `template_imports.apply_mount_index_set_rename` (from `parse._load_ref_data`), Rust `template_imports.rs::apply_mount_index_set_rename` (from `ref_loading.rs::resolve_value`), TypeScript `template-imports.ts::applyMountIndexSetRename` (from `ref-loading.ts::resolveRefDocument`), Go `template_rename.go::applyMountIndexSetRename` (from `subsystem_ref.go::resolveSubsystemMap`). Each uses a NEW walk rather than the §9.7.7 declaration walk, because in a whole component `from` also names a data source / coupling endpoint; the mount walk keys off POSITION and never rewrites a bare string on its own account. Fixtures: `tests/valid/mount_rename_{column_grid,atm_column,soil_column,two_columns}.esm` and `tests/invalid/template_imports/mount_rename_unknown_index_set.esm`. **Top-level `models.<k>` `{ref}` mount**: Python honours the field there too (both mount forms share `_load_ref_data`); Julia and Rust splice that form's leaf raw and drop its `index_sets` (nothing to rename until #198 item 3 lands), and TypeScript and Go do not inline the form at all.
