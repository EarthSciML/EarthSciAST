...
| SCHEMA-03-A-006 | Library MUST accept the DEPRECATED `"op": "aggregate"` spelling, normalize each such node to the canonical `"op": "faq"` at the WIRE BOUNDARY (before the version gates and schema validation, so nothing downstream of the loader — `emit` included — sees the alias), and report the `deprecated_op_alias` warning exactly ONCE per document per alias, naming the node count. Consequence: `emit(load(aliased))` is byte-identical to the canonical document, so a document authored with the alias is upgraded exactly once. Removed at esm 2.0.0. Gate: `tests/conformance/deprecated_op_alias/` | esm-spec §4.2, CONFORMANCE_SPEC §7, docs/content/rfcs/faq-node-rename.md | Yes | schema |
| SCHEMA-03-A-007 | Library MUST REJECT `"op": "arrayop"` BY NAME at the wire boundary with `removed_op`. It is not a deprecated alias — it was removed at esm 0.8.0. Rejecting by name is required rather than incidental: `arrayop` matches the `op` pattern, so esm-spec §4.2 would otherwise admit it as an OPEN rewrite-target op and the document would load silently, failing much later (or never) as `unlowered_operator`. Gate: `tests/invalid/faq/arrayop_op_removed.esm` | esm-spec §4.2, CONFORMANCE_SPEC §7, docs/content/rfcs/faq-node-rename.md §5.4 | Yes | schema |
| SCHEMA-03-A-008 | A document spelling the node `faq` MUST declare `esm: 1.1.0` or later, the same version-gate rule the top-level `solver` block follows. The rule is TRANSITIVE through lowering: a document whose EXPANDED form contains `faq` — a template-library consumer whose imported rules lower to one — declares 1.1.0 even when its authored bytes carry no `faq` node | esm-spec §2.2.4, §4.2 | Yes | schema |
...
> at all, since a `faq` body reading `t` for a forcing term is ordinary and would
...
| BEHAV-04-D-004 | An index-set `size` a mounted leaf cannot close MUST merge SYMBOLICALLY and be closed by the mounting document, at either mount form and whatever §9.7 machinery the leaf carries — a mount edge is not the last scope that could close the name (§9.7.6 site 5). It MUST NOT depend on whether the leaf declares `metaparameters` or imports a template library. A ROOT document has no enclosing scope and stays strict (`metaparameter_unbound`) | esm-spec.md §4.7, §9.7.6 | Yes | validation |
| BEHAV-04-D-008 | The §4.7 index-set merge MUST run AFTER the mounting document's own §9.7.6 close ("subsystem refs resolve post-close"), and the incoming contribution MUST be folded against that already-closed metaparameter environment BEFORE the deep-equal comparison. A contribution whose free names that environment does not supply stays symbolic rather than erroring. Merging before the close, or comparing unfolded, rejects two IDENTICAL declarations (an assembly and its leaf both declaring `NLEV: 40` and `lev: {size: "NLEV"}`) and makes a concrete restatement of a symbolic leaf axis an error while a verbatim one is idempotent. INLINING is a separate step and MUST stay early enough for the §9.6.3 fixpoint to lower through a mounted component | esm-spec.md §4.7, §9.7.6 | Yes | validation |
...
| BEHAV-04-D-005 | A mount edge MAY carry `index_set_rename`, applied to the mounted document AFTER it resolves in its own scope and BEFORE its `index_sets` merge, rewriting the registry key and every reference to it inside the mounted document (`of` parent lists, `{"from"}` ranges, `wrt`/`dim`/`var` and bare-axis `integral` bounds, `where` `shape`, `faq` `join.<i>.on` key columns that name a renamed axis, variable/parameter `shape`, `Assertion.coords` keys, `DataSourceSelectAxis.gated_by`). Absent or empty ⇒ identity | esm-spec.md §4.7 "Mount-edge index-set renaming" | Yes | validation |
| BEHAV-04-D-006 | An `index_set_rename` key that names no index set of the RESOLVED mounted document MUST be rejected with `subsystem_index_set_rename_unknown_name`; a non-dotted-identifier target with `template_import_rename_invalid`; two keys onto one target with `template_import_rename_collision`. A mount FORM the binding cannot apply the field at MUST be refused with `subsystem_index_set_rename_unsupported_mount_form`, never silently ignored — a requirement no binding now meets at a `subsystems.<k>` or top-level `models.<k>` edge because all five APPLY the field at both, leaving only the top-level `reaction_systems.<k>` form to refuse (Julia, the one binding that mounts that form) | esm-spec.md §4.7, §9.6.6 | Yes | validation |
| BEHAV-04-D-009 | A binding that does not implement a §4.7 mount form MUST refuse an entry at that form with `mount_form_unsupported` at the entry's pointer, never load it as an empty or unresolved component. Currently the top-level `reaction_systems.<k>` `{ref}` form: Julia mounts it; Rust, TypeScript, Go and Python refuse it at `/reaction_systems/<k>` | esm-spec.md §4.7 "Two mount forms, one mechanism", §9.6.6 | Yes | validation |
| BEHAV-04-D-007 | A MOUNTED document may itself be an assembly, whose own `models.<k>` entries are `{ref}` mount edges: those MUST be resolved — in that document's directory, against that document's own registry — before its single top-level system is extracted and before its `index_sets` merge upward, at BOTH attachment points, because which point mounted a file cannot change what that file is. Chains of any depth compose. Cycle detection MUST be path-scoped (pushed on enter, popped on exit) so one component file may be mounted under several keys, and a cycle along the current path MUST name the cycle rather than recurse without bound. A binding that resolves one level deep splices the leaf's unresolved `{ref}` edge in where a component belongs — a silently malformed document, which is the failure this rule exists to stop | esm-spec.md §4.7 | Yes | validation |
...
>
> **BEHAV-04-D-003, top-level `models.<k>` mount form (2026-09-06, issue #198 item 3)**: the merge now runs at BOTH model mount forms in every binding that mounts top-level refs. Python already did (`resolve_model_refs` calls `_merge_subsystem_index_sets` on the mounted leaf). Julia and Rust merged only `function_tables`/`data_sources`/`enums` at the top-level edge and silently dropped the leaf's axes, so an assembly had to redeclare them or lean on its §9.7.10 injected grid library — fixed by `_merge_native_index_sets!` in `resolve.jl` and the `merge_subsystem_index_sets` call in `ref_loading.rs`'s `inline_toplevel_model_refs`, both at the raw/native pre-pass layer where that inliner runs, both reusing the §4.7 deep-equal-or-`subsystem_index_set_conflict` rule verbatim. Shared fixtures: `tests/valid/toplevel_ref_index_set_merge.esm` and `tests/invalid/template_imports/toplevel_ref_index_set_conflict.esm`, which mount the SAME leaf as the subsystem-edge pair (`tests/valid/subsystem_mesh_lib.esm`) through the other attachment point — a differential test of the two mount forms. Since TypeScript and Go gained the top-level mount form (issue #280), the pair and `toplevel_ref_metaparameter_axis.esm` sit in the shared valid and invalid sets, and all five bindings' corpus sweeps agree on them. None of the three restates a leaf's symbolic axis, so none reaches the divergence recorded at the end of this note. Runners: `pkg/EarthSciAST.jl/test/template_imports_test.jl`, `pkg/earthsci-ast-rs/tests/template_imports_conformance.rs`, `pkg/earthsci-ast-py/tests/test_model_ref.py`. ~~**Known scope limit at the top-level form (Julia, Rust)**: the inliner is a raw pre-pass that never closes or folds the mounted leaf's `metaparameters` (the edge's `bindings` are not read there either), so a leaf `interval` whose `size` is still a metaparameter expression is SKIPPED rather than merged — merging it unfolded coerces to `Int`/`i64` and fails with an internal error, not a diagnostic. Python, which closes and folds at both forms, merges it correctly. Closing this needs the top-level mount edge to run the §9.7.6 site-3 close/fold the subsystem edge already runs; it is not part of BEHAV-04-D-003.~~ **Closed in Rust (2026-09-10)**: `inline_toplevel_model_refs` in `ref_loading.rs` now runs the SAME §4.7 edge pipeline `resolve_value` runs at a `subsystems.<k>` edge — library gates, edge `bindings` folded against the mounting scope, §9.7.10 form-A injection, the leaf's metaparameter close and fold, lower + expand, `index_set_rename`, then the nested walk — so step (3)'s merge sees a resolved mounted document and the fold guard is gone. Three consequences closed together, all one root cause: (1) the merge reaches the interpreter BUILD, not only the validator; (2) the leaf's `metaparameters` close at the edge per §9.7.6 site 3, so `bindings` at this mount form are read at last; (3) `index_set_rename` applies here instead of being refused with `subsystem_index_set_rename_unsupported_mount_form` — §4.7 says that refusal "closes when that gap does". A `size` still open after the leaf's OWN close names something the leaf does not declare. A leaf with NO §9.7 machinery has no close to run, so such a `size` merges symbolically and the mounting document's §9.7.6 close resolves it, which keeps an assembler-scoped axis loading; a leaf with ANY machinery closes strictly at the edge, where the same name is `metaparameter_unbound`. Two refusals are therefore NEW at this mount form, both matching Python: that `metaparameter_unbound`, and a `subsystem_index_set_conflict` when an assembly restates the leaf's symbolic axis with the CONCRETE number it folds to (`size: 7` against the leaf's `size: "n_rows"`) rather than verbatim -- on the no-machinery path the merge's deep-equal test is syntactic. Rust runners: `tests/toplevel_mount_edge_pipeline.rs` (fold, edge `bindings`, the conflict on a just-folded axis, restate-or-delete idempotence, round-trip fixed point), `tests/mount_index_set_rename.rs`, `tests/template_imports_conformance.rs`. **Closed in Julia (2026-09-11)** the same way: `resolve.jl::_inline_toplevel_model_refs!` runs the same pipeline `_resolve_subsystem_ref` runs, the `_native_index_set_is_folded` guard is gone, and the loader-API metaparameters are threaded from `_load_document` so they can backfill a mounted leaf's close. `toplevel_ref_metaparameter_axis.esm` is now 3-of-3 (all three merge `lev` = 4). Julia runner: `pkg/EarthSciAST.jl/test/toplevel_mount_edge_pipeline_test.jl`, plus `mount_index_set_rename_test.jl` and `template_imports_test.jl`. One PRECEDENCE rule is load-bearing and was a live bug in review of the Rust change: only the LOADER-API bindings (site 4) backfill a leaf's own close, for the names the leaf declares. The mounting document's own unbound `default`s are site 5 — its own close — and forwarding them would let an assembler's unrelated `NLEV: default 12` resize a leaf axis the edge never bound, giving a different answer from the `subsystems.<k>` mount of the same leaf. The mounting document's FULL environment is used for one thing only: folding an edge `bindings` EXPRESSION. **Known Rust/Julia-vs-Python divergence on the no-machinery path, found porting this and NOT fixed**: when an assembly both declares `n_rows` and RESTATES the leaf's symbolic axis verbatim (`size: "n_rows"`), Rust and Julia merge on the raw document before their root close (deep-equal, loads; the axis then folds to 7) while Python merges after its root close (`size: 7` against `size: "n_rows"`, `subsystem_index_set_conflict`). The same document therefore loads in two bindings and is refused in the third — a VERDICT split, wider than the value-only split already recorded in `toplevel_mount_edge_pipeline.rs`'s strictness-boundary docstring, which asserts the verdicts agree. Deleting the restatement (the terse spelling) loads in all three. Which side is right is a spec question about where the §4.7 merge sits relative to the mounting document's own §9.7.6 close. No shared fixture restates a leaf's symbolic axis, so no corpus-swept file reaches this split. **Issue #236 pin**: `tests/valid/mount_forwarded_axis_toplevel_beside_nested.esm` mounts a column whose `lev` is imported from a grid library and sized by a forwarded `NLEV` at a top-level `models.<k>` beside a nested `subsystems.<k>` mount of the same column at the same size, and must merge. `tests/invalid/template_imports/mount_forwarded_axis_size_conflict.esm` is the same pair at 59 and 4, and must refuse with `subsystem_index_set_conflict`. The Rust binary from before this note's 2026-09-10 change refuses both with `template_import_index_set_conflict`. **Follow-up closed in Rust (2026-09-12, issue #311)**: running the edge pipeline at this form exposed an ORDERING defect the old raw pre-pass had masked. Both edges consumed the §9.7.10 form-A injection and ran the §9.6.3 fixpoint BEFORE resolving the leaf's own nested `{ref}` mounts, so a rewrite-target inside a component the leaf itself mounts escaped every rule and reached evaluation as `unlowered_operator` — unlowerable from any mount edge, by any spelling, while the SAME library declared in the leaf's own `expression_template_imports` lowered it. §9.7.10 defines the injection as "as if the target had added those entries to the end of its own `expression_template_imports`", so the two must agree; §4.7's "before validation or any other processing" says which way — the nested walk moves ahead of the injection and the fixpoint, at both mount forms, with nested edge `bindings` folding against the leaf's own closed environment (`root_metaparameter_env`) as a standalone load gives them. Before this, the reporting assembly's seven-stage WSM6 microphysics chain was red at exactly the four stages carrying a nested subsystem. Shared fixtures `tests/fixtures/mount_edge_injection_nested/` (the self-import CONTROL that pins §9.7.10's equivalence, the same leaf mounted at each form, and a four-file chain that puts the rewrite target TWO mounts below the edge, so the reordering is pinned as recursive rather than one-level); Rust runner `tests/template_imports_conformance.rs::mount_edge_injection_reaches_the_leafs_own_nested_subsystem`, confirmed failing on the unpatched tree at the `models.<k>` case with the control passing. **Ported to Go and TypeScript (2026-09-13, owner ruling: do not ship Rust-only)**. `subsystem_ref.go::resolveSubsystemMap` and `ref-loading.ts::resolveRefDocument` now run the leaf's own nested mounts — of BOTH §4.7 forms — ahead of the injection and the fixpoint, against a LEAF-LOCAL index-set registry reconciled with the leaf's own axes after the fold (`foldNestedIndexSets` in each), and `applyMountIndexSetRename` in each takes the nested-contributed names and holds them out of the edge's vocabulary. Porting exposed the SAME pre-existing defect in both, independent of #311: a §9.7.10 form-A injection at a top-level `models.<k>` mount edge lowered NOTHING, even with the rewrite-target in the mounted leaf's own equation, because each binding's root machinery pass walks `models.<k>` as COMPONENTS and ends every one with a `delete` of `expression_template_imports` — it cannot tell a mount edge from a component — so the edge reached the ref resolver with its injection already gone. Go now snapshots the edges off the AUTHORED text (`extractTopLevelModelRefEdges` moved ahead of `resolveAndLowerJSONCapturing`); TypeScript skips a bare-`{ref}` entry in that pass, by the `ref`-with-no-`variables` discriminator `isTopLevelMountEdge` uses. Fixture `root_toplevel_flat.esm`-shaped probes went 1 surviving operator → 0 in both.
>
> **MEASURED five-binding state after those ports (2026-09-13)**, loading each fixture and counting surviving `{"op": "input_x"}` nodes — `nested_self_import.esm` / `nested_subsystem_mount.esm` / `nested_model_mount.esm`: **Rust** 0 / 0 / 0; **Go** 1 / 0 / 0; **TypeScript** 1 / 0 / 0; **Julia** 1 / 1 / 1; **Python** 2 / 1 / 1. Both MOUNT-EDGE fixtures now agree in Rust, Go and TypeScript. The one still standing in Go and TypeScript is the CONTROL — a document's own component declaring the library in its own `expression_template_imports` and mounting the rewrite-target through a `subsystems.<k>` `{ref}` — and it is NOT an edge-pipeline question at all. **What blocks the control: the ROOT document's ref resolution runs AFTER the root's own §9.6.3 fixpoint in Go, TypeScript and Julia, and BEFORE it in Rust.** Go: `LoadPath` calls `LoadString` (schema → `resolveAndLowerJSONCapturing` → typed decode) and only then `resolveSubsystemRefsWithMeta` on the typed `*ESMFile`; the component's `expression_templates` do not survive to the typed `Model` at all (only the document-scope `ESMFile.ExpressionTemplates` does), so there is nothing to re-lower against afterwards. TypeScript: `loadString` then a separate `resolveSubsystemRefs`. Julia: `_load_parsed` then `resolve_subsystem_refs!`. Moving root ref resolution ahead of the root machinery is what fixes it. **AUTHORIZED, and sequenced BEHIND pull request #317 (2026-09-13).** The first reading of this — that hoisting resolution necessarily drags the §4.7 index-set merge to BEFORE the root's §9.7.6 close, the Rust/Julia-vs-Python divergence recorded earlier in this note — was WRONG, because it treated two steps as one. #317 rules that the merge happens AFTER the mounting document's §9.7.6 close, with the incoming contribution folded against that closed environment before the deep-equal comparison, and separates the two steps so that is expressible: **inlining** a referenced document's content, which must happen early enough for the §9.6.3 fixpoint to lower through it (what these ports need), and **merging** its `index_sets` into the registry, which happens after the close with the fold. The root reorder therefore hoists only the INLINE and leaves the merge where #317 puts it. #317 lands first because it touches the same four resolvers; this work rebases onto it. **Consequence, TEMPORARY, to be aware of until the root reorder lands**: in Go and TypeScript a mount-edge INJECTION now reaches the leaf's nested subsystem while the leaf's OWN declared import still does not, which inverts the §9.7.10 equivalence those two clauses assert — the injected form is momentarily STRONGER than the self-declared one it is defined to equal. Rust has both; Julia and Python have neither. It is accepted on the ground that the alternative — not porting the edge — would leave Go and TypeScript disagreeing with Rust AND with each other on the two mount-edge fixtures, which is strictly worse; it closes when the root reorder does. **Julia: NOT ported, stopped deliberately.** Its `subsystems.<k>` resolution is entirely on the TYPED tree (`resolve_subsystem_refs!` → `_resolve_refs_in_file!` → `_resolve_subsystem_ref` over `SubsystemRef`/`Model`/`IndexSet`), built by `_load_parsed` from the raw dict, while the machinery and the fixpoint run on that raw dict beforehand. There is no raw-dict `subsystems.<k>` resolver to hoist: `_inline_toplevel_model_refs!` is a raw-dict edge pipeline but walks only `models.<k>`. Porting therefore means either a second full edge pipeline on native dicts — the duplication that function's own docstring says the binding refuses — or restructuring the typed load so refs inline before `_load_parsed`, which is the same root-level move above. **RULED (2026-09-13): the restructured typed load**, not a native-dict pipeline. Duplicating cycle detection, library rejection, bindings fold, injection, machinery, fixpoint, rename, merge, extraction and splicing on the raw dict was declined explicitly, agreeing with that docstring. Sequenced behind #317 for the same reason as the reorder. Both mount-edge fixtures stay red in Julia until then. **Also ruled (2026-09-13): `resolve_subsystem_refs!` becomes a THIN WRAPPER over that native pass** — render the typed file to native, resolve, re-parse — matching the Rust design API_SPEC.md §8 item 14 already documents (`resolve_subsystem_refs_raw` is the implementation; the typed entry point is built over it). The "one pipeline" reading was taken over leaving a second code path alive, accepting the round-trip fidelity risk that carries: `pkg/EarthSciAST.jl/test/corpus_fidelity_test.jl` is the detector, and Julia's `OrderedDict` key-order guarantees plus `IndexSet`'s `members_raw` / `member_factor` are the likely casualties. Losslessness is to be VERIFIED explicitly when that step lands, not inferred from a green suite. **Python: out of scope by ruling**, and behind by TWO gaps that both predate this change and are independent of mount edges. First, its §9.6.3 fixpoint does not reach a component's SUBSYSTEMS at all: a one-argument rule declared in a component's own `expression_templates`, with the rewrite-target in a subsystem written INLINE in the same component, is left standing by `earthsci_ast.load_path` while Rust, TypeScript and Go all lower it — so for Python the mount-edge ordering is the second problem, not the first. Second, it does not apply a ZERO-argument `match` rule even on a component's own equation: `{"params": [], "match": {"op": "input_x", "args": []}, "body": "x_in"}` declared locally alongside an equation carrying `input_x()` comes back unlowered from `load_path`, and `expand_document` does not lower it either, while the other four do; the same rule with one parameter lowers correctly in Python, so the gap is specific to the empty-`args` match pattern — which is exactly the shape a §9.7.10 coupling-target discretization library uses. Until Julia and Python are brought over, a document of this shape resolves to DIFFERENT content depending on which binding loads it — recorded here rather than silently carried. **ROOT REORDER LANDED in Go, TypeScript and Julia (2026-09-14); the temporary §9.7.10 inversion above is CLOSED.** Measured surviving `{"op": "input_x"}` nodes on `nested_self_import.esm` / `nested_subsystem_mount.esm` / `nested_model_mount.esm`: **Rust 0 / 0 / 0; Go 0 / 0 / 0; TypeScript 0 / 0 / 0; Julia 0 / 0 / 0; Python 2 / 1 / 1** (Python out of scope by ruling, for the two gaps recorded above). Each binding reached it differently, and every route keeps both the merge-order rule (a §4.7 contribution merges after the mounting document's §9.7.6 close) and §8.9.4's (the static `extent` check sees names collected from the authored tree): **Go** — `LoadPath` collects the mount-declared set with the existing `rootMountContext` on the AUTHORED bytes, inlines the root's mounts and STAGES their `index_sets` (`inlineRootRefsRaw`), raises the `faq` floor on the inlined text, hands the set to `LoadString` through the unexported `withAuthoredMountDeclared`, and merges after the close (`applyStagedIndexSets`); `LoadString`'s public contract that it does not resolve refs is unchanged. **Julia** — `_load_document` inlines the root's `subsystems.<k>` mounts natively before `_load_parsed`, below its existing authored-tree collection, staged with the top-level form's and merged after the close; each mount edge (`_resolve_mount_edge_core`, now shared by both forms) collects first, inlines its own nested mounts second, and lands their contributions after its own close. **TypeScript** — `loadPath` keeps its contract that it does not resolve refs; `resolveSubsystemRefs` re-runs the §9.6.3 pass over each root component that had mounted content spliced in, using the rules kept on `componentTemplates`. A rewrite pattern cannot span the root/inlined boundary (a pattern is one expression tree, and splicing adds the mounted component's equations beside the component's own, referenced only by scoped name), so the second pass cannot differ from one that saw both; the re-run's idempotency is pinned by `nested_self_import_own_target.esm`, whose component also has a rewrite-target in its OWN equation — `own = 3*x_in` and `sd.y = 2*x_in` in Rust, Go and TypeScript alike. **Still open in Julia, deliberately not landed:** (a) making `resolve_subsystem_refs!` a thin render/resolve/re-parse wrapper over the native pass — `EsmFile` is an immutable struct and the function mutates its argument in place, so a re-parse cannot write back a changed `esm` (the raised floor) or turn a `nothing` block into a dictionary; re-parsing an already-inlined document re-runs the whole load pipeline, the `faq` version gate included; and deleting the typed walk would drop Julia's remote-ref resolution, which `subsystem_ref_test.jl` exercises and the native pass deliberately skips; (b) deleting the top-level `reaction_systems.<k>` refusal — that form is still NOT routed through the shared core but an older raw pre-pass that defers the leaf's §9.7 resolution to the root and never merges index sets, so removing the refusal before routing it would make `index_set_rename` silently ignored there, the silence §4.7 forbids. **RULED (2026-09-14): kept**, see BEHAV-04-D-009 below. **Withdrawn note (2026-09-14):** an earlier version of this entry said Rust emits an authored `3.0` as `3` after a rule lowers it. Measured through each binding's canonical emitter (Rust `esm convert`, Go `ToJSON`, TypeScript `toJson`, Python `to_json`, Julia `to_json`), ALL FIVE write `3.0` as `3`, and they do so on a control document with no rule and no lowering at all. It is a shared emitter convention, not a Rust divergence and not an effect of lowering. **The reordering's own cross-binding cost, found in review and closed in the same change**: moving the nested walk ahead of the fixpoint also puts the nested mounts' `index_sets` into the leaf's registry BEFORE this edge's `index_set_rename` runs, which the other four never see (they resolve nested refs after the rename). Unguarded, that silently widened the rename's vocabulary: a `{"ref": "./leaf.esm", "index_set_rename": {"lev": "soil_lev"}}` whose `lev` is declared ONLY by a component the leaf itself mounts was accepted and renamed in Rust, while Julia, Python, TypeScript and Go all refuse it with `subsystem_index_set_rename_unknown_name` — a VERDICT split, at both mount forms. esm-spec §4.7 is explicit that they are right ("step 2 covers what THIS referenced document declares and imports, and an axis reaching the registry through a mount *nested inside* the referenced document is renamed (or not) at that nested edge"), so `apply_mount_index_set_rename` now takes the set of names the nested walk contributed and holds them out of the edge's vocabulary. Fixtures `tests/fixtures/mount_edge_rename_nested_scope/` (the grandchild that declares `lev`, the leaf that only mounts it, and the refusing edge at each mount form); Rust runner `tests/mount_index_set_rename.rs::a_mount_edge_rename_cannot_name_an_axis_a_nested_mount_contributed`. Verified 2026-09-13 that all five bindings now produce the identical code and message on both fixtures. **A second rule settled while integrating the merge-order change (2026-09-14): a §4.7 merge folds against the closed metaparameter environment of whatever REGISTRY it lands in.** §4.7 said the contribution folds against "the mounting document's already-closed metaparameter environment" without saying which document that is when the contribution lands in a mounted leaf's own registry rather than the root's, and the three bindings had answered differently — Rust the leaf's closed environment, TypeScript the bare edge `bindings`, Go the root's. Measured on `tests/fixtures/mount_merge_fold_env/`, which makes the two candidates disagree on purpose (a grandchild sizes an axis `"n_lev"` and carries no §9.7 machinery so the name survives its own load unfolded; the leaf declares `n_lev: 4` and mounts it; the assembly declares an unrelated `n_lev: 9` and mounts the leaf): **Rust 4, Go 9, TypeScript 9** before, all three **4** after. Folding against the assembly's environment let an unrelated same-named metaparameter one level up silently resize an axis the leaf owns — the same failure §9.7.6 site 4's declared-names filter prevents for `bindings`. esm-spec §4.7 now states the rule; one test each in Rust, Go and TypeScript. **Julia is a separate pre-existing gap on this fixture**: it raises `MethodError(Int64, ("n_lev",))`, because its typed `IndexSet.size::Int` cannot hold a symbolic size at all on the no-machinery path. Not a fold-environment disagreement, and not fixed here. **The other end of the same rule, RULED and now normative (2026-09-13)**: when the leaf declares an axis AND a component it mounts declares a deep-equal one of the same name, the enclosing edge's rename reaches BOTH — the §4.7 merge has already made them one axis, so the resolved document keeps one registry entry under the new name and the nested component's `shape` is re-pointed. Renaming only the half the leaf spelled itself would leave the mounted subtree naming an axis its own registry no longer holds. esm-spec §4.7 now states vocabulary and reach as two separate questions so the two ends cannot be collapsed into one test. Fixtures `rename_scope_shared_leaf.esm` + `rename_scope_shared_axis{,_toplevel}.esm` alongside the `rename_scope_nested_only_axis*` pair; runners in Rust, Go and TypeScript. Measured 2026-09-13: **Rust, Go and TypeScript** all give `index_sets = {soil_lev}` with no surviving `lev` reference, at both mount forms; **Julia and Python** still give `{lev, soil_lev}` with the nested component on `lev`, because they merge the nested declaration back in after the rename. That is the same nested-walk ordering the #311 port has not reached in those two — it closes with it, not separately. The VOCABULARY end already agrees in all five: `rename_scope_nested_only_axis{,_toplevel}.esm` is `subsystem_index_set_rename_unknown_name` everywhere.
> **BEHAV-04-D-007 — mounting an assembly (2026-09-12, found reviewing the item-4 port)**: the mount form composes with itself. Measured on the pre-fix tree with a three-file chain (root mounts an assembly, the assembly mounts a component): at the TOP-LEVEL form **Rust** and **Julia** resolved it and named a cycle; **Python** raised `AttributeError: 'dict' object has no attribute 'name'`; **Go** raised `ambiguous_subsystem_ref` "resolves to a component that is not a model", which it is; and **TypeScript** loaded clean, splicing the leaf's UNRESOLVED `{ref}` edge in as the component and returning a mount cycle as a resolved document pointing at itself. At the `subsystems.<k>` form only **Rust** composed — Go and TypeScript spliced in the unresolved edge, Python and Julia failed — because the four resolve a mounted document with a path that never runs the top-level inliner. All five now compose at both forms, each by calling its own one edge-pipeline implementation rather than a second copy: Go `subsystem_ref.go::inlineNestedTopLevelModelRefs` (from `resolveSubsystemMap`, before `extractSingleSystemRaw`), TypeScript `ref-loading.ts::inlineNestedTopLevelMounts` (from both `inlineTopLevelModelRef` and `walkSubsystemRefs`), Python `parse.py::resolve_model_refs` recursing on the parsed leaf and from `_resolve_model_subsystems::resolve_ref`, Julia `resolve.jl::_load_local_ref` running `_inline_toplevel_model_refs` on the referenced document with the caller's `visited` set. Fixtures: `tests/valid/mount_chain_inner.esm` is a one-model assembly; `mount_chain_outer.esm` mounts it at the top-level form and `mount_chain_via_subsystem.esm` at the subsystem form, and both must land the innermost component's variables and carry the INNER edge's renamed axis (`soil_lev`) up. Cycles are pinned per binding rather than in the shared corpus: the bindings' circular-reference diagnostics do not share a code (Rust `toplevel_model_ref_unresolved`, TypeScript `circular_dependency`, Go an uncoded error, Julia/Python their own exception types), and `expected_errors.json` demands one code from all five. **Remaining limit**: Julia's top-level inliner reads its targets with `isfile`, so an assembly reached over `http(s)://` does not compose there; no binding disagrees about a document, only about how far a remote chain reaches.

> **BEHAV-04-D-005/006 — mount-edge `index_set_rename` (2026-09-06, issue #198 item 4; RFC `docs/content/rfcs/mount-edge-index-set-renaming.md`)**: implemented in **all five** at the §4.7 subsystem edge — Julia `template_imports.jl::apply_mount_index_set_rename` (from `resolve.jl::_lower_and_coerce`, with the map carried on `SubsystemRef.index_set_rename`), Python `template_imports.apply_mount_index_set_rename` (from `parse._load_ref_data`), Rust `template_imports.rs::apply_mount_index_set_rename` (from `ref_loading.rs::resolve_value`), TypeScript `template-imports.ts::applyMountIndexSetRename` (from `ref-loading.ts::resolveRefDocument`), Go `template_rename.go::applyMountIndexSetRename` (from `subsystem_ref.go::resolveSubsystemMap`). Each uses a NEW walk rather than the §9.7.7 declaration walk, because in a whole component `from` also names a data source / coupling endpoint; the mount walk keys off POSITION and never rewrites a bare string on its own account. Fixtures: `tests/valid/mount_rename_{atm_column,soil_column,two_columns}.esm` and `tests/invalid/template_imports/mount_rename_unknown_index_set.esm`. The two column components declare `lev` in their own top-level `index_sets` rather than importing it from a shared grid library, because the `tests/valid/**` corpus sweeps in four bindings run reference resolution on the RAW document, where an axis contributed by an unresolved `expression_template_imports` edge is invisible; renaming an import-contributed axis is normative but currently has no regression fixture. **Top-level `models.<k>` `{ref}` mount**: the field is normative there too (esm-spec §4.7 "Where it applies"), and **all five bindings now apply it**, each by running its ONE §4.7 edge-pipeline implementation at both attachment points rather than a second copy of it — Python `parse._load_ref_data` (shared by both forms from the start), Rust `ref_loading.rs::inline_toplevel_model_refs` (2026-09-10), Julia `resolve.jl::_inline_toplevel_model_refs!` (2026-09-11), TypeScript `ref-loading.ts::inlineTopLevelModelRef` (which calls the same `resolveRefEdge`/`resolveRefDocument` the subsystem walk calls), Go `subsystem_ref.go::inlineTopLevelModelRefs` (which hands the edge to `resolveSubsystemMap` through a one-entry holder map). Before that, TypeScript left a top-level `{ref}` stub in place and Go decoded it into an EMPTY `Model` — `ESMFile.Models` is a `map[string]Model` — so in Go the mount, the leaf, and the rename were all discarded with no diagnostic at all. Shared fixtures for the form: `tests/valid/mount_rename_two_columns_toplevel.esm` and `tests/invalid/template_imports/mount_rename_unknown_index_set_toplevel.esm`, each the twin of its `subsystems.<k>` sibling at the other attachment point — the differential test that catches a binding implementing the field at one form only. Julia's top-level `reaction_systems.<k>` inliner (`resolve.jl::_inline_toplevel_reaction_system_refs!`) is the one place the refusal survives: it is still a raw pre-pass that defers the leaf's §9.7 resolution to the root document, so it has no resolved mounted document to rename and REFUSES the edge with `subsystem_index_set_rename_unsupported_mount_form`. ~~No other binding mounts that form at all, so there the field is neither applied nor refused. **RULED (2026-09-13): that refusal is to be DELETED.**~~ **Superseded (2026-09-14)**: measuring the form across bindings showed Julia is the only binding that mounts it at all, so routing it through the shared core and deleting the refusal would have widened a disagreement rather than closed one. The refusal and Julia's pre-pass are KEPT unchanged, and the other four now refuse the whole form (BEHAV-04-D-009 below), so the field is not silently ignored anywhere. Every binding defines the code, because the code table is cross-language uniform. Refusing rather than ignoring is what item 3 made load-bearing: the top-level form merges the leaf's axes, so an ignored rename would merge the leaf under its PRE-rename axis names, silently, in the one place this field exists to make loud.

> **BEHAV-04-D-009 — top-level `reaction_systems.<k>` `{ref}` is refused where it is not implemented (2026-09-14, ruled while closing pull request #313)**: no `.esm` file in the repository uses the form; the only uses are documents built inline in Julia's `reaction_system_ref_test.jl`. Measured on the pre-change tree with an `esm: 1.0.0` document mounting a one-reaction-system leaf at `reaction_systems.Chem`: **Rust** rejected it only incidentally (`missing field species`, a typed-decode failure, not a diagnostic about the form); **TypeScript** loaded it and left the `{ref}` stub in place; **Go** and **Python** loaded an EMPTY reaction system (0 species) with no diagnostic; **Julia** loaded it with both species, through its raw pre-pass (`resolve.jl::_inline_toplevel_reaction_system_refs!`), which does not merge the leaf's `index_sets` and refuses `index_set_rename` there. **Ruled**: keep Julia's refusal and pre-pass unchanged; make the other four refuse the form explicitly. New code `mount_form_unsupported` (esm-spec §9.6.6), because no existing code fits — `subsystem_index_set_rename_unsupported_mount_form` refuses one field at a form a binding DOES mount. After: Rust (`ref_loading.rs::refuse_toplevel_reaction_system_refs`, called at the top of `walk_top_level`; the `esm` producer reports it through `StructuralErrorCode::MountFormUnsupported`), TypeScript (`ref-loading.ts`, the reaction-systems loop of `resolveSubsystemRefsSync`), Go (`parse.go::refuseTopLevelReactionSystemRefs`, from `LoadString` after schema validation) and Python (`parse.py::_refuse_toplevel_reaction_system_refs`, from `_load_data`) all produce `mount_form_unsupported` at `/reaction_systems/Chem`; Julia's output on the six recreated documents is byte-identical before and after. Each refusal is keyed on an entry carrying a string `ref` and no `species`, so an inline reaction system is untouched. Fixture: `tests/fixtures/mount_form_unsupported/toplevel_reaction_system_ref.esm` (+ `reaction_system_leaf.esm`), with one test per binding (Rust `tests/mount_index_set_rename.rs::a_toplevel_reaction_system_ref_is_refused_loudly`, TypeScript `src/mount-index-set-rename.test.ts`, Go `mount_index_set_rename_test.go::TestTopLevelReactionSystemRefIsRefusedLoudly`, Python `tests/test_mount_index_set_rename.py::test_toplevel_reaction_system_ref_is_refused_loudly`). **Why it is not in `tests/invalid/` + `expected_errors.json`**: Julia loads the document, and `scripts/compare-conformance-outputs.py` requires every `tests/invalid/**` file to be rejected by every binding, with no per-binding exemption, and `expected_errors.json` has no field to express one. Rather than invent such a mechanism, the pair lives under `tests/fixtures/`, which the corpus sweep does not walk. **Follow-up, not built**: implementing the form in Rust (parse support), TypeScript, Go and Python (real resolution through each binding's shared edge core), after which Julia's pre-pass can be routed through its own shared core and the rename refusal deleted. **Two further pre-existing splits found while measuring, recorded and NOT fixed**: (1) a leaf declaring BOTH a model and a reaction system, mounted at a top-level `models.<k>` `{ref}`, is rejected by TypeScript and Go as "multiple top-level systems" and accepted by Rust, Python and Julia; (2) five of the six documents in Julia's `reaction_system_ref_test.jl` declare `esm: 0.8.0`, which Julia loads and Rust, TypeScript and Python reject as an unsupported major version.
...
### BEHAV-06-B: Inline-Test Assertion Semantics (pinned §6.6.3/§6.6.4/§6.6.5 conventions)
| ID | Requirement | Spec Reference | Testable | Test Category |
|---|---|---|---|---|
| BEHAV-06-B-001 | An assertion's rank is decided PER ASSERTION from the declared `shape` of the variable it names, not from the component: a pointwise assertion (no `coords`, no `reduce`) on a variable declared with a non-empty `shape` MUST name an element (`u[1]`), and `coords` / `reduce` on a variable declared without a shape is rejected. Validators reject both with `assertion_rank_mismatch`, for a target the asserting component declares by a bare name; a dotted target is left to the runtime. Gate: `tests/invalid/assertion_rank_mismatch_*.esm`, `tests/valid/inline_test_static_check_spellings.esm` | esm-spec.md §6.6.5 | Yes | validation |
...
| BEHAV-06-B-012 | The RELATIVE tolerance bound MUST scale by `max(\|actual\|, \|expected\|)` — the larger of the two magnitudes — not by `\|expected\|` alone, and a conforming runtime MUST NOT add an `ε` floor to that scale. §6.6.3 stated the rule three ways: the normative box and the schema's `Tolerance` description gave an `\|expected\|`-only DENOMINATOR with an `ε` floor, while the finiteness rationale in the same section reasoned from `max(\|∞\|, \|expected\|)`; all three executing bindings (Julia `isapprox`, Python `_check_assertion`, Rust `check_assertion`) implemented the symmetric form, so #193 settled the SPEC as symmetric and dropped the floor (with the bound written as a product there is no division to protect, and a floor diverges on subnormals: `a=1e-320, e=0, rel=0.5` passes with `ε=1e-300` and fails without). The verdicts differ only inside `rel·\|e\| < \|a−e\| ≤ rel·\|a\|` — an overshoot of order `rel` — which no fixture in any category reaches, so `assertion_nonfinite` cannot gate it. Gate: the SHARED data-only category `assertion_tolerance` (CONFORMANCE_SPEC §5.37, `tests/conformance/assertion_tolerance/`) — 43 `(actual, expected, rel, abs) → verdict` cases fed straight to each binding's own predicate, six of which change verdict under the `\|expected\|` reading, seven under the `abs + rel·\|expected\|` sum form and three under an `ε` floor; required of Julia, Python, Rust and TypeScript, with Go `scope_excluded` (it has no predicate anywhere) and asserting that exclusion. The per-binding unit tests `assertion_tolerance_symmetry_test.jl` (Julia), `test_relative_bound_is_symmetric_in_actual_and_expected` (Python) and `relative_bound_is_symmetric_in_actual_and_expected` (Rust) remain as each binding's own statement of the discriminating case. The ~12 ad-hoc assertion evaluators that scaled by `\|expected\|`, summed the two bounds or carried `ε` floors are FIXED (#223): every one now calls the binding's own `check_assertion` / `_check_assertion` | esm-spec.md §6.6.3, CONFORMANCE_SPEC §5.20, §5.37 | Yes | conformance (shared) + unit (per-binding) |
| BEHAV-06-B-013 | An inline `reference`'s free variables are the field's DIMENSION NAMES: for a variable shaped over index sets, each `shape` entry is bound at every grid point to the 1-based position along its axis (the index space `coords` reads). A reference mentioning a dimension name free MUST be evaluated per cell as if wrapped in an `aggregate` whose output indices are the dimension names; one mentioning none (a literal, a parameter expression, an explicit gather, a gather that rebinds the dimension name as its own loop symbol) MUST be evaluated as written. A node's `wrt` is a differentiation target, not a free mention, and MUST NOT by itself trigger the wrap; a dimension name the BUILD-TIME SCOPE ALREADY BINDS MUST be a fault, since wrapping would silently shadow that other meaning with the cell index. The clash scope is the WHOLE build-time scope, in two halves: the resolved scalar parameters, AND the build-time ARRAY names (a materialized state-free array observed, an inline `const` array, a shaped parameter's inline column, a provider-/loader-injected field), each half with its unambiguous bare aliases. All three executing bindings left the name unbound. Julia/Python/Rust: FIXED (`bind_dimension_names`). The ARRAY half was checked by none of them, and Julia is where that mattered: it hands its cellwise evaluator the build's `const_arrays`, so a const array named after a shape index set (an array `lev` over the index set `lev`) was silently rebound to the cell's 1-based index while Python and Rust merely wrapped — issue #226, FIXED in all three via `array_scope_names` / `_array_scope_names` (Julia and Python pass `const_arrays` + `setup_arrays`, Rust `setup_arrays`). Gate: `tests/conformance/pde_inline_reference_dimension_names/`; the `wrt` clause and BOTH halves of the clash are gated per binding on `bind_dimension_names` directly, since a clash fixture would be a document every binding rejects | esm-spec.md §6.6.5, CONFORMANCE_SPEC §5.30 | Yes | simulation |
| BEHAV-06-B-014 | A dotted `parameter_overrides` / `initial_conditions` key MUST resolve to the LONGEST of its dotted suffixes that is a flattened name (rule 2), the trailing segment being tried last, so the §4.6 fully-qualified `M.sub.A` binds a build's `sub.A` and `M.A` binds a bare `A`; a key none of whose suffixes is a name stays unknown (`Missing.solo`). Rule 3 is the SAME relationship read the other way and MUST admit a key SHORTER than the name: the key is a dotted SUFFIX of exactly ONE flattened name, of which the bare trailing segment is only the shortest case, so `sub.g` binds a flattened `P.sub.g` just as `g` does. Bare-only left the mirror-image divergence open — `sub.g` reached NEITHER rule against the flattened build Python and Julia produce and raised `UnknownParameterError`, while Rust's single-model array build carries the parameter as `sub.g` outright and took the same key as an exact hit (issue #227, FIXED in all three). A suffix carried by two or more names is AMBIGUOUS and MUST name every candidate (`dup.k` over `Left.dup.k` / `Right.dup.k`), never tie-broken; `Missing.sub.g` stays unknown, since rule 3 drops nothing and no name ends with it. Every LEADING segment rule 2 drops MUST name a component or subsystem the document declares — the dropped prefix is a §4.6 qualifier, so `Doc.Left.solo` with no component `Doc`, and a mistyped `Missng.M.pert_amp`, are UNKNOWN rather than silently suffix-matched onto the name they happen to end with. And TWO NON-EXACT keys resolving to ONE name (`solo` + `Doc.Left.solo`; `A.M.g` + `B.M.g`) MUST be reported naming the variable and every colliding key — only one override can take effect, so ranking them exact-before-bare-before-dotted turns a diagnosable authoring mistake into a wrong answer; an EXACT hit is never part of a collision and wins outright. Rust (`canonicalize_override_keys` + `namespace_scope`), Python (`check_parameter_override_keys` / `resolve_override_raw` + `namespace_scope`), Julia (`_canonicalize_override_keys` + `_override_namespaces`, plus the `initial_conditions` path in `simulate.jl`): FIXED. Rust additionally resolves a self-qualified EQUATION reference `M.sub.g` in a single-model array build to its local spelling (esm-spec §4.6). Gate: `tests/conformance/override_key_diagnostics/` — case `Doc.Left.solo` pinned UNKNOWN (a deliberate reversal of the case as first committed), and the fixture's mounted subsystems pinning all three spellings of `Left.sub.g` (`Left.sub.g` / `sub.g` / `g`) as resolving to one trajectory, `dup.k` and `k` as ambiguous, and `Missing.sub.g` as unknown; rule 2's positive path is not expressible in a flattening fixture and is gated per binding by the `P.sub.g` subsystem-override tests, the collision by each binding's verbatim-message unit test | esm-spec.md §6.6.2, §4.6, CONFORMANCE_SPEC §5.31 | Yes | simulation |
| BEHAV-06-B-015 | A **scalar** on a SHAPED variable MUST be BROADCAST over the variable's whole declared grid — the one value applies to every element — for a shaped PARAMETER's declared `default` and for a test's `parameter_overrides` alike, not only for the shaped UNKNOWN whose scalar `default` already seeded every state cell. The choice of channel is the binding's; the broadcast is not, so a binding MUST NOT leave the value in a one-f64-per-parameter vector that cannot express it. All three executing bindings failed this, each in its own vocabulary: Rust indexed a scalar (`eval_index` → `NaN`) and every solve died at `t = 0` with `Exceeded maximum number of nonlinear solver failures`, naming no parameter; Python broadcast a BARE read but raised 'index applied to scalar value' on `index(p, k)`; Julia refused the document with `E_TREEWALK_UNSUPPORTED_SHAPE`. A scalar `parameter_overrides` value MUST additionally OUTRANK the declared `default` it replaces in EITHER spelling — Julia ranked the scalar arm after the array-`default` arm and silently dropped such an override. Rust (`lower_inline_array_parameters` + the `inline_tests` routing), Python (`_build_numpy_rhs`'s parameter loop → `loader_arrays`), Julia (`_register_inline_array_parameters` → `const_arrays`): FIXED. Gate: `tests/conformance/shaped_parameter_broadcast/` | esm-spec.md §6.3, §6.6.2, CONFORMANCE_SPEC §5.32 | Yes | simulation |
| BEHAV-06-B-016 | A component's inline `tests` MUST NOT cross a MOUNT EDGE: when a document mounts a component by a top-level `models` / `reaction_systems` `{ref}` (§4.7 / §9.7.10), the mounted component's `tests` MUST be dropped at load, and a runner whose target is the mounting document MUST NOT run them. A leaf's assertions are claims about the leaf under its OWN standalone conditions, which the mounting document may legitimately change — a `variable_map` replacing one of its parameters with another component's state (§10), a mount-edge `expression_template_imports` lowering it under a different discretization (§9.7.10), a mount-edge `bindings` closing a metaparameter at another value (§9.7.6) — so re-running them there checks a claim their author never made and reports a correct component as broken (767 rows in the assembly that reported this as #198 item 2). Coverage is not lost: a directory-wide run reaches the leaf's own file, where the same assertions run standalone. Dropped at the MOUNT rather than skipped in the runner, so every consumer of the assembled document — `esm pretty`, `esm validate`, a downstream tool — agrees about what that document asserts. Rust (`ref_loading.rs::inline_toplevel_model_refs`), Python (`parse.py::resolve_model_refs`) and Julia (`resolve.jl::_inline_toplevel_model_refs!` + `_inline_toplevel_reaction_system_refs!` — the only binding with a top-level reaction-system mount): FIXED. TypeScript (`ref-loading.ts::inlineTopLevelModelRef`) and Go (`subsystem_ref.go::inlineTopLevelModelRefs`) gained the top-level mount on 2026-09-12 and drop the mounted component's `tests` at it, so the DOCUMENT half of the rule holds in all five; neither has an inline-test runner, so the runner half is still out of scope there. Go drops them on the raw map before the typed decode, so a leaf's tests are not merely discarded but never interpreted — the mounting document is not the one that gets to say whether they are well-formed. A `subsystems` `{ref}` mount needs no rule: a test's target is the TOP-LEVEL component it lives in, so a subsystem's `tests` are not run by the mounting document either. SHOULD: a runner printing a per-file summary NAMES the mount edges it found (Rust `esm test`, Julia `run_esm_tests`), so the components a run did not assert on are visible rather than silently absent. Gate: `tests/conformance/mounted_component_tests/` — shared fixtures driven per binding (`mounted_component_tests.rs`, `test_mounted_component_tests.py`, `mounted_component_tests_test.jl`) in the manner of `subsystem_mount_join_names.rs`; the fixture couples the mounted leaf to `k = 5` so a binding that re-runs its `u(1) = 1/e` assertion misses by two orders of magnitude, and the assembly's own passing test is the control that stops a binding passing by running nothing | esm-spec.md §6.6, §4.7 | Yes | simulation |
| BEHAV-06-B-017 | Tolerance resolves PER FIELD across FOUR levels — assertion / test / model / the implementation default: `abs` and `rel` each take their value from the innermost level that DECLARES that field, so a level declaring only one bound MUST NOT mask the other from an enclosing level (`model {rel: 1e-6}` + `assertion {abs: 1e-9}` resolves to `(1e-6, 1e-9)`, not `(0, 1e-9)`). All three simulation bindings returned the first non-null block WHOLE and defaulted its missing field to 0, so such an assertion ran with NO relative bound at all — an author's declared tolerance silently discarded. No other conformance tier could see it: every fixture elsewhere declares exactly one tolerance block, and one block merges the same way under either rule, so the three bindings agreed on the wrong answer. Two rules §6.6.4 left implicit are settled with it: an explicit `0` is a DECLARATION ("no bound of this kind") that stops the fallthrough, while a missing key or a JSON `null` is an ABSENCE that falls through (`tests/fixtures/recurrence/*.esm` pin exactness with a model-level `{rel: 0, abs: 0}`, which the other reading would silently run at `rel = 1e-6`); and the implementation default `rel = 1e-6` is the FOURTH MERGE LEVEL, not a terminal fallback — it supplies whichever bound is still undeclared after levels 1-3, so an `abs`-only assertion resolves to `rel = 1e-6` and `rel: 0` is the only spelling of "absolute bound and nothing else". The 63 authored `abs`-only blocks governing the 69 corpus assertions that would otherwise have acquired that band carry an explicit `rel: 0`, so no shipped assertion's enforced bound moves. Resolution is MONOTONE against the old rule (no bound ever tightens), so an assertion can flip fail→pass under the change but never pass→fail; each gate asserts that rather than claiming it. Gate: `tests/conformance/tolerance_resolution/` (data-only, no integrator), CONFORMANCE_SPEC §5.21 | esm-spec.md §6.6.4 | Yes | unit (per-binding) |
| BEHAV-06-B-018 | An observed with a declared `shape` whose right-hand side is a SCALAR — a literal, a scalar parameter, or an expression that evaluates to a scalar, such as an `ifelse` whose predicate is constant — MUST have every element of its declared shape, each carrying that value (esm-spec §4.3.4: a scalar declares no index sets, so it replicates along all of the result's). The value's rank MUST NOT replace the declared shape. All three executing bindings failed this: Rust stored the scalar as a 0-d array, so an assertion reported `array state '<name>' has no cells in var_map` and a derivative reading the observed failed the solve at `t = 0`; Python did the same for a right-hand side that is a scalar as written; Julia refused the document with `E_TREEWALK_UNSUPPORTED_SHAPE` because its elementwise array-observed fold admitted neither a scalar right-hand side nor `ifelse`. Rust (the wholesale observed rule carries its declared extents, filled by the interpreter and the tape), Python (`_materialize_observeds`), Julia (`_fold_elementwise_array_observeds` + `ifelse` flagged foldable): FIXED. Gate: `tests/conformance/shaped_observed_scalar_broadcast/` | esm-spec.md §4.3.4, CONFORMANCE_SPEC §5.41 | Yes | simulation |
| BEHAV-06-B-019 | An inline-test assertion's `variable` MAY be a scoped reference (schema `Assertion.variable`; issue #263), resolved by the rule the asserting component's own equations use: a dotted name whose HEAD is a subsystem key of the asserting component is relative to it (`Leaf.u` in `Host` with a `Leaf` subsystem is `Host.Leaf.u`), and any other dotted name is document-absolute (§4.6). Every lookup the assertion makes — the trajectory row, an array state's cells, an observed's field, the declared shape a `coords` map is checked against — MUST be made against the component that OWNS the name, and MUST match only under a spelling that owner can hold: the whole component path (`Host.Leaf.key`), or, in a document with a single top-level component, the path below the root (`Mid.Leaf.key`). The bare-name and unique-suffix fallbacks that resolve an UNSCOPED assertion name the ASSERTING component's own element and MUST NOT be reached for a scoped one — each of them otherwise answers with a different component's column when the owner's own field is not a build product. Before the fix a `coords` / `reduce` assertion on a mounted leaf's field errored with `variable 'Leaf.key' is not declared in model 'Host'` in both mount forms, and a pointwise one on a name two components answer to read whichever row sorted first. Julia (`inline_tests.jl::_resolve_asserted_name`), Python (`inline_tests.py::_resolve_asserted_name`) and Rust (`inline_tests.rs::resolve_asserted_name`): FIXED. Rust also chose its simulation backend (`file_has_array_ops`, `file_has_spatial_model`, `has_differential_equations`) from top-level models only, so a document whose only derivative or array lives in a `subsystems` mount failed to build; those now ask the question of the whole mount tree (`simulate_array::model_tree_any`). TypeScript and Go have no inline-test runner and do not resolve `Assertion.variable` at validation, so there is nothing to diverge. Gate: `tests/conformance/scoped_assertion_variable/` — shared fixtures driven per binding (`scoped_assertion_variable.rs`, `test_scoped_assertion_variable.py`, `scoped_assertion_variable_test.jl`) over a top-level mount, a `subsystems` mount, a THREE-level mount chain, a document where both readings of `Leaf` exist and the subsystem must win, and one where both exist and only the top-level component's field is a build product | esm-spec.md §6.6.3, §4.6 | Yes | simulation |

### BEHAV-06-C: Observed-Definition LHS Spellings (esm-spec §6.3.1, CONFORMANCE_SPEC §5.36)
| ID | Requirement | Spec Reference | Testable | Test Category |
|---|---|---|---|---|
| BEHAV-06-C-001 | Both LHS spellings that DEFINE an unknown — bare (`y ~ f(…)`) and indexed (`y[i] ~ f(…)`, which defines the whole array `y`) — MUST be accepted at EVERY rank. §6.3.1 states the criterion semantically (the defining form is read through the LHS's base name, so "an arrayed definition is observed exactly as its scalar counterpart is") and explicitly retires the earlier bare-LHS-only wording. A binding MAY keep a narrower strict-`y ~ f(…)` set alongside `observed_unknowns` for inlining (Python's `inlined_unknowns`), but that set governs HOW an observed is eliminated — a scalar substitutes into its consumers, an array materializes into a buffer they index — never WHETHER the document is accepted. Julia: FIXED — every owner bucket of the tree-walk build (elementwise fold, promoted-arrayop inline set, bare-alias registration, clip-ring discovery) tested the SYNTACTIC bare LHS, so an ARRAY-shaped observed written the indexed way matched none of them, fell through to the partition's geometry-ring gate, and was refused with `E_TREEWALK_UNSUPPORTED_SHAPE` on a document Rust and Python both ran (issue #232); `_normalize_indexed_observed_lhs` now rewrites the spelling into the bare one UPSTREAM of every classifier (not in the runner, which AGENTS.md forbids from dispatching on rule shape, and not by widening the gate, which would admit the shape with no owner). Rust: already conforming. Python: FIXED on `main` by PR #276 (commit `bac8a6197`, cherry-picked from #237's `57b72acad`) — `flatten._normalize_indexed_observed_lhs` mirrors the Julia normalizer and `classification._base_name` sees through the same shell so `observed_definitions` credits the indexed spelling; the category scored 7/14 before it and 14/14 after. A recognition-boundary difference between the two normalizers was found and closed at the same time: `distinct` is a BOOLEAN whose absence MEANS false (esm-schema `ExpressionNode.distinct`: "Absent ⇒ false … exactly as today"), so only a TRUE one is a set-semantics shell — Julia tested `distinct !== true`, Python tested the field's PRESENCE and so declined `"distinct": false`, the very same node, and answered `0.0`. Python's three symptoms — an indexed-LHS array observed answering `0.0` to an assertion, one whose rhs is a PER-CELL body silently dropped from the ODE RHS, one feeding a WHOLE-ARRAY derivative dropped the same way — share ONE root cause, and it is the SAME wrong substitution as Julia's: `flatten._collect_model` read observed-ness from `classification.inlined_unknowns`, the strict `y ~ f(…)` set §6.3.1 sanctions for inlining specifically, used as the classification. Two bindings, one error, diagnosed independently. Gate: `tests/conformance/pde_inline_observed_indexed_lhs/` — the observeds are asserted DIRECTLY (which is what states the §6.3.1 contract rather than the bindings' current intersection), as a controlled pair of one STATE-FREE and one STATE-DEPENDENT, so fixing one routing path only does not pass; the states they drive are asserted alongside, so answering the observeds while dropping them out of the dynamics still fails | esm-spec.md §6.3.1 | Yes | simulation |
...
> against the committed `tests/valid/faq/join_filter.esm`;
...
> the shared fixture `tests/valid/faq/join_on_data_columns.esm`, whose inline
...
> Gates: the shared fixture `tests/valid/faq/join_on_self_join.esm`
...
>
> Julia has adopted `tests/valid/faq/join_on_data_columns.esm` into
> `test/faq_conformance_test.jl`; Python's `test_faq_conformance.py`
...
> `run_inline_tests_filtered`, which the CLI's `--filter` now calls instead of filtering the
...
| FORMAT-08-A-014 | The discovered count binds at §9.7.6 site 4, and that site reaches any document in the assembly that declares the name — this document OR one it MOUNTS at either §4.7 mount form, transitively. A mounting document MUST NOT have to restate a metaparameter its leaf already declares | esm-spec.md §8.9.4, §9.7.6 | Yes | validation |
| FORMAT-08-A-015 | An `extent.metaparameter` no document in the assembly declares MUST be rejected at LOAD with `template_import_unknown_name`, not deferred to the sample at build time. "Declared" is exactly what §9.7.6 site 4 binds over, read off the AUTHORED documents: this document's own, a MOUNTED document's (either §4.7 form, transitively), and a name an imported template library declares and the edge leaves unbound (§9.7.6 site 2 re-export). The check MUST be skipped for a document already in resolved shape — no unresolved `{ref}` mount, every interval `size` a concrete integer — so a binding that re-loads its own resolved document is not told it is invalid | esm-spec.md §8.9.4, §9.6.6 | Yes | validation |
| FORMAT-08-A-016 | A leaf mounted at a `subsystems.<k>` edge and the same leaf mounted at a top-level `models.<k>` edge MUST be sized from the discovered extent IDENTICALLY (§4.7 "Two mount forms, one mechanism"); a binding that backfills the loader-API bindings at one attachment point and not the other sizes one from the data and the other from the leaf's placeholder default, silently | esm-spec.md §4.7, §8.9.4 | Yes | behavioral |
...
> **CLOSED — the opposite direction, and all five skip sets now agree
> (2026-09-07, EXPR-09-E-008)**: the note above left the skip sets *not* yet
> identical. **Go alone** carried `op` / `id` / `expect_cadence`, so with `max`
> bound to 3 the node `{"op": "max", …}` stayed intact in Go and became
> `{"op": 3, …}` in Julia, Python, Rust and TypeScript — which then died in the
> typed load with a raw "cannot unmarshal number into `op`" rather than a
> diagnostic (Go's audit note G12). §9.7.6 substitutes only in *expression
> positions*, and an operator name is not one, so Go's behavior was the correct
> one and the other four gained the three entries.
>
> Both directions are now fixed the same way: each binding derives its skip set
> from ONE kind-tagged structural-field table, with the same five kinds
...
> `semiring`, `manifold`, `fn`, `table`, `side`, `attrs`, `members`, `from_faq`
> — were substituted by **every** binding, Go included: with `max` bound to 3,
> `{"op": "aggregate", "reduce": "max", …}` became `"reduce": 3` in all five.
> That was a shared GAP rather than a conformance divergence, but it is the same
> bug in kind — none of those fields is an expression position — so the skip set
> is `protected ∪ axis ∪ node-header ∪ registry`. Note the scope of "Go is the
> reference" from the note above: Go was right about `op` / `id` /
> `expect_cadence` and wrong here along with everyone else.
>
> Every structural kind is now in the skip set except two: `bound` — an
> `integral` `lower`/`upper` (§4.2), which genuinely IS an expression position
> and must keep substituting — and the positional `from` / `of`. The fixture's
> fourth equation covers the four registry fields that live on an
> ExpressionNode and need no extra document machinery (`reduce`, `semiring`,
> `fn`, `attrs`), each beside a bare metaparameter name in a genuine expression
> position, so a too-BROAD skip set fails as loudly as a too-narrow one.
| ID | Requirement | Spec Reference | Testable | Test Category |
|---|---|---|---|---|
...
| EXPR-09-E-008 | Metaparameter names in expression positions MUST substitute as integer literals with no further folding. Substitution is per-FIELD: the **axis-naming scalars** `wrt`, `dim` (§4.9.1) and `integral`'s `var` (§4.2), the **node-header fields** `op`, `id` and `expect_cadence`, and the **op-parameterizing registry fields** `reduce` / `semiring` / `manifold` / `fn` / `table` / `side` / `attrs` / `members` / `from_faq`, are NOT expression positions and MUST be copied verbatim even when a bound metaparameter spells them exactly. An `integral` `lower`/`upper` bound IS an expression position and MUST substitute | esm-spec.md §9.7.6, §9.7.7 | `tests/conformance/expression_templates/metaparam_axis_name_collision/` | expression |
...
| EXPR-09-F-004 | `rebind` MUST rewrite free variable names in bodies/matches (incl. `faq` `args` and `index` gathers) and ragged `offsets`/`values`; dotted targets are §4.6 scoped references | esm-spec.md §9.7.7 | Yes | expression |
...
| SERIAL-05-A-003 | Arithmetic-op operand literal types are by value, not reader-inferred storage: an **integer operand of `+ - * / ^ neg` stays an integer literal inside AND outside a `faq` `expr` body**. A binding whose JSON reader widens a bare integer token to float (Julia `JSON3` structural inference) MUST re-apply rule 1 when building the AST | CONFORMANCE_SPEC.md §5.5.3.1 rule 3 | Yes | serialization |
...
**Conformance fixture:** `tests/valid/faq/coordinate_int_ratio_spacing.esm` (registered in `tests/conformance/round_trip/manifest.json`, tag `ess-aggregate-intdiv`) authors cell-centre spacing `(i − 1/2)·(1/N)` with the exact integer ratio `{op:"/",args:[1,8]}` / `[1,16]` INSIDE a `faq` `expr` body; all five bindings (julia/python/rust/ts/go) MUST round-trip it byte-identically, and the inline `tests` block pins the values (`coordA[1]=0.0625`, i.e. `0.5·(1/8)`; `coordB[1]=0.03125`). Julia regression guard: `parse_test.jl` "Integer ratio inside aggregate stays integer (§5.5.3.1)" loads a `cos(pi·…)`-nested document that provokes the JSON3 widening and asserts the ratio re-serializes as `1/8`, not `1.0/8.0`.
